...
import { calculateAdditionalChangeover } from "@/utils/changeoverCalc";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
// SINGLE SOURCE OF TRUTH for a sim row's effective avail date.
// This exactly mirrors the preview "Avail Date" cell so that the preview display,
// the apply snapshot, the persisted target_avail_date, and the chronological sort
// all use the same value (WYSIWYG). Returns the raw value to persist/sort on:
// an ISO date string, the 'stock_sufficient' sentinel, a non-date label, or null.
function effectiveAvailRaw(row) {
  // Cat D / Cat B with an N10D-inferred target date → the inferred date
  if (
    (row._category === "D" || row._category === "B") &&
    row._inferredTargetDate
  )
    return row._inferredTargetDate;
  // Cat D without an inferred date (or explicit sentinel) → stock sufficient
  if (row._category === "D" || row.target_avail_date === "stock_sufficient")
    return "stock_sufficient";
  // Everything else → the preview avail date (may be ISO or a non-date label)
  const pv = row._previewAvailDate;
...
// Preview production time = effective volume / run_rate (mirrors Dashboard's
// calcProductionHours so the preview matches the applied schedule exactly).
// Never returns a bogus 0.00 for a nonzero-volume row that has a valid run rate.
function computePreviewProductionHours(row) {
  // Respect an explicit manual override of production hours.
  if (row.production_hours_manual) {
    return row.production_hours != null ? parseFloat(row.production_hours) : null;
  }
  // Manual form (M) has no production time.
  if (row.form === "M") {
    return row.production_hours != null ? parseFloat(row.production_hours) : null;
  }
  // Prefer the order's STORED production_hours when present and non-zero. This is
  // exactly what the applied schedule + edit view display (they use the order's own
  // run_rate). The orders fed to this modal are KB-enriched, so row.run_rate may be
  // the KB master rate rather than the order's stored rate — recomputing from it
  // would diverge from the schedule (e.g. 148/20=7.40 vs the stored 148/19.098=7.75).
  const stored = row.production_hours != null ? parseFloat(row.production_hours) : null;
  if (stored != null && stored > 0) {
    return stored;
  }
  // Stored missing/zero → recompute from effective volume / run rate so the preview
  // never shows a bogus 0.00 for a nonzero-volume row that has a valid run rate.
  const rr = parseFloat(row.run_rate);
  const vol = getEffectiveVol(row);
  if (rr > 0 && vol > 0) {
    return parseFloat((vol / rr).toFixed(2));
  }
  // Cannot compute — fall back to any stored value.
  return stored;
}

function getProdMs(row) {
  const _ph = computePreviewProductionHours(row);
  const ph = _ph != null ? _ph : 0;
  const co =
...
//
// Start-timing reset rules (applied to the preview only):
//  • First order (index 0): always keeps its start date/time as the schedule anchor.
//  • Non-first orders:
//    – Only carry start_date/start_time if the value is "manual"
//      (i.e. set via localEdits in this session, or row.start_date_manual === true in DB).
//    – Non-manual carried-over values are reset (shown as blank) to avoid stale timing.
//    – Preserved manual values are flagged with _manualStartMisaligned when they fall
//      before the previous order's cascaded completion time.
function buildSimRows(rows, localEdits = {}) {
  ...
  return rows.map((row, i) => {
    ...
    // Determine if this order's start date/time is "manual" —
    // either edited within this preview session, or flagged in DB.
    const hasLocalStartDateEdit = edits.startDate !== undefined;
    const hasLocalStartTimeEdit = edits.startTime !== undefined;
    const isManualStartDate = hasLocalStartDateEdit || (row.start_date_manual === true);
    const isManualStartTime = hasLocalStartTimeEdit || (row.start_time_manual === true);

    // Raw values (from session edits or DB)
    const rawStartDate = hasLocalStartDateEdit ? edits.startDate : (row.start_date || null);
    const rawStartTime = hasLocalStartTimeEdit ? edits.startTime : (row.start_time || null);

    // For non-first orders: only display manual timing; reset system-carried values.
    const startDate = isFirstOrder ? rawStartDate : (isManualStartDate ? rawStartDate : null);
    const startTime = isFirstOrder ? rawStartTime : (isManualStartTime ? rawStartTime : null);

    // Debug log 1: Reset information for every row
    const hadDisplayedStartDate = !!(hasLocalStartDateEdit ? edits.startDate : row.start_date);
    const hadDisplayedStartTime = !!(hasLocalStartTimeEdit ? edits.startTime : row.start_time);
    const startDateReset = !isFirstOrder && hadDisplayedStartDate && !isManualStartDate;
    const startTimeReset = !isFirstOrder && hadDisplayedStartTime && !isManualStartTime;
    console.debug('[Auto-Sequence Start Timing Reset]', {
      orderId: row.id,
      isFirstOrder,
      hadDisplayedStartDate,
      hadDisplayedStartTime,
      wasManualStartDate: isManualStartDate,
      wasManualStartTime: isManualStartTime,
      startDateReset,
      startTimeReset,
    });
    ...
    if (completionOverride) {
      ...
      completion = calcSimCompletion(
        effectiveStartDate,
        effectiveStartTime,
        computePreviewProductionHours(row),
        effectiveChangeover,
      );
    ...
    // Check if preserved manual start is misaligned with the new cascade
    let manualStartMisaligned = false;
    const hasPreservedManual = !isFirstOrder && startDate && (isManualStartDate || isManualStartTime);
    if (hasPreservedManual && prevCompletion) {
      try {
        const sd = new Date(startDate + 'T00:00:00');
        const timeStr = startTime || '08:00';
        const tp = String(timeStr).match(/(\d+):(\d+)/);
        if (tp && !isNaN(sd.getTime())) {
          sd.setHours(parseInt(tp[1], 10), parseInt(tp[2], 10), 0, 0);
          manualStartMisaligned = sd.getTime() < prevCompletion.getTime();
        }
      } catch {
        // ignore parse errors
      }
    ...
    // Debug log 2: preservation of manual timing
    if (hasPreservedManual) {
      console.debug('[Auto-Sequence Manual Timing Preservation]', {
        orderId: row.id,
        manualStartDate: startDate,
        manualStartTime: startTime,
        preservedInPreview: true,
      });
    }

    // Debug log 3: misalignment warning
    if (hasPreservedManual && manualStartMisaligned) {
      console.debug('[Auto-Sequence Manual Timing Warning]', {
        orderId: row.id,
        manualStartDate: startDate,
        manualStartTime: startTime,
        warningShown: true,
        reason: 'manual_timing_misaligned_with_resequenced_schedule',
      });
    }
    ...
    return {
      ...
      _isManualStartDate: isManualStartDate,
      _isManualStartTime: isManualStartTime,
      _manualStartMisaligned: manualStartMisaligned,
...
export default function AutoSequenceModal({
  ...
  const handleConfirmApply = async () => {
    ...
    const sequence = simRows.map((row, i) => ({
      ...
      isManualStartDate: row._isManualStartDate === true,
      isManualStartTime: row._isManualStartTime === true,
      ...
      // Single source of truth: the exact effective avail date the preview displays.
      // The apply writes and sorts on this so the schedule is WYSIWYG.
      appliedAvailDate: effectiveAvailRaw(row),
      production_hours: computePreviewProductionHours(row),
    }));

    // Debug: verify the preview production time uses Volume / Run Rate and never
    // shows a bogus 0.00 for a nonzero-volume row with a valid run rate.
    simRows.forEach((row) => {
      const volume = getEffectiveVol(row);
      const runRate = parseFloat(row.run_rate) || 0;
      const previewProductionTime = computePreviewProductionHours(row);
      console.debug('[Auto-Sequence Preview Production Time Check]', {
        line: lineName ?? null,
        feedmill: feedmillName ?? null,
        orderId: row.id,
        volume,
        runRate,
        expectedProductionTime: runRate > 0 ? volume / runRate : null,
        previewProductionTime,
        previewMatchesExpected:
          runRate > 0
            ? Math.abs((previewProductionTime ?? 0) - (volume / runRate)) < 0.01
            : false,
      });
      console.debug('[Auto-Sequence Preview Invalid Time Detection]', {
        line: lineName ?? null,
        feedmill: feedmillName ?? null,
        orderId: row.id,
        volume,
        runRate,
        previewProductionTime,
        invalidZeroTimeForNonzeroVolume:
          volume > 0 && runRate > 0 && (previewProductionTime ?? 0) === 0,
      });
    });
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 nexfeed-force-light"
        onClick={onClose}
      >
        <div
                        ...
                        {preSequenceSnapshot.map((row, idx) => (
                          <tr key={row.id} className="auto-sequence-row-before">
                            ...
                            <td className="as-col-material">
                              <div title={getGenFgCode(row, pmxSplitRules) || ""} style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "#2e343a" }}>
                                {getGenFgCode(row, pmxSplitRules) || "-"}
                              </div>
                              {row.kb_sfg_material_code && <div className="as-sub-text" style={{ fontSize: 10, color: "#6b7280", marginTop: 2 }}>{row.kb_sfg_material_code}</div>}
                            </td>
                            ...
                            <td className="as-col-prod">
                              <div style={{ fontSize: 12, fontWeight: 600, color: "#1a1a1a" }}>
                                {formatProductionHours(computePreviewProductionHours(row))} hours
                              </div>
                        ...
                        {(() => {
                          ...
                          return (
                            <tr>
                              ...
                              <TH style={{ width: 140, textAlign: "left" }}>Material Code</TH>
                                ...
                                >
                                  {(draggableProvided, snapshot) => (
                                    ...
                                    >
                                      ...
                                      >
                                        <div
                                          title={getGenFgCode(row, pmxSplitRules) || ""}
                                          style={{
                                            overflow: "hidden",
                                        ...
                                        {row.kb_sfg_material_code && (
                                          <div style={{ fontSize: 10, color: "#6b7280", marginTop: 2 }}>{row.kb_sfg_material_code}</div>
                                        )}
                                      </td>
                                      ...
                                      >
                                        {editingCell?.id === row.id && editingCell?.field === "startDate" ? (
                                          ...
                                          <span style={{ display: "inline-flex", alignItems: "center", gap: 3, color: "#2e343a", whiteSpace: "nowrap" }}>
                                            {fmtDateShort(row._simStartDate)}
                                            {row._manualStartMisaligned && (
                                              <AlertTriangle
                                                size={11}
                                                color="#f59e0b"
                                                title="Start date/time was manually set and should be reviewed after auto-sequence."
                                                style={{ flexShrink: 0 }}
                                              />
                                            )}
                                          </span>
                                      ...
                                      >
                                        ...
                                        >
                                          ...
                                          >
                                            {(() => {
                                              const _av =
                                                effectiveAvailRaw(row);
                                              if (_av === "stock_sufficient")
                                                return "Stock sufficient";
                                              if (!_av) return "-";
                                              return isAvailDateValid(_av)
                                                ? fmtDateShort(_av)
                                                : _av;
                                            })()}
                                          </span>
                                      ...
                                      <td
                                        style={{
                                          ...
                                          textAlign: "left",
                                          background: row._hasCompletionOverride ? "rgba(var(--nexfeed-primary-rgb), 0.07)" : "transparent",
                                      ...
                                      >
                                        {editCompletionId === row.id ? (
                                          ...
                                          <div>
                                            <div style={{ fontWeight: 500, color: "var(--nexfeed-primary)", whiteSpace: "nowrap", fontSize: 12 }}>
                                              {row._simCompletionStr}
                                            </div>
                                            <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
                                              <span style={{ fontSize: 10, color: "var(--nexfeed-primary)", fontWeight: 500 }}>Overridden ✏</span>
                                              <button
                                                onMouseDown={(e) => e.preventDefault()}
