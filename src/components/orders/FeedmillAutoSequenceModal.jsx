...
function combineOnLine(orders, targetLine) {
  ...
  Object.values(groups).forEach((grp) => {
    ...
    // ── Hard guardrail: generated and source orders cannot combine ──────────
    // Split the group by class; only same-class sub-groups may be combined.
    const genGrp = grp.filter(_isGen);
    const srcGrp = grp.filter(o => !_isGen(o));
    if (genGrp.length > 0 && srcGrp.length > 0) {
      for (const gen of genGrp) {
        for (const src of srcGrp) {
          console.debug('[Manual/Auto Combine Rejected]', {
            orderAId: gen.id,
            orderBId: src.id,
            combineAttemptType: 'auto',
            rejected: true,
      ...
      // Continue as two separate same-class groups below; skip this mixed group
      [genGrp, srcGrp].forEach((subGrp) => {
        if (subGrp.length < 2) return;
        const totalMT = subGrp.reduce((s, o) => s + (parseFloat(o.total_volume_mt) || 0), 0);
        const combinationAllowed = totalMT <= MAX_COMBINE_MT;
        console.debug('[Order Combine Cap Check]', { combineCapMT: MAX_COMBINE_MT, candidateCombinedVolumeMT: totalMT, withinCap: combinationAllowed });
        console.debug('[Order Combine Cap Validation]', { previousCapMT: 180, newCapMT: MAX_COMBINE_MT, combinationAllowed });
        if (!combinationAllowed) return;
        const sorted = [...subGrp].sort((a, b) => (parseFloat(b.total_volume_mt) || 0) - (parseFloat(a.total_volume_mt) || 0));
        const lead = { ...sorted[0] };
        lead.total_volume_mt = totalMT.toFixed(1);
        lead.production_hours = (subGrp.reduce((s, o) => s + (parseFloat(o.production_hours) || 0), 0)).toFixed(2);
        lead._isCombined = true;
        lead._combinedFrom = subGrp.map((o) => ({
        ...
        lead._combinedFromLines = [...new Set(subGrp.map((o) => o.feedmill_line || targetLine))];
        const changeoversSaved = subGrp.length - 1;
        placementLog.push({
          type: "combined", product: lead.item_description,
          materialCode: lead.material_code, ordersCount: subGrp.length,
          totalVolume: totalMT, toLine: targetLine,
          fromLines: subGrp.map((o) => o.feedmill_line || targetLine),
          changeoversSaved, timeSaved: changeoversSaved * 0.17,
          individualVolumes: subGrp.map((o) => ({ fromLine: o.feedmill_line || targetLine, volume: parseFloat(o.total_volume_mt) || 0 })),
        ...
        subGrp.forEach((o) => placed.add(o.id));
        result.push(lead);
      });
      return; // mixed group handled via sub-groups above
    }
    // ── End guardrail ───────────────────────────────────────────────────────
...
function StatBadge({ label, value, color = "default" }) {
  const colors = {
    ...
    orange:  { bg: "rgba(var(--nexfeed-primary-rgb), 0.08)", border: "rgba(var(--nexfeed-primary-rgb), 0.35)", textColor: "var(--nexfeed-primary)", valColor: "var(--nexfeed-primary-dark)" },
...
/* ── After row (drag-and-drop) ── */
function FMAfterRow({ order, prio, provided, snapshot, destinationLine, pmxSplitRules = [] }) {
    ...
    <>
      ...
      >
        ...
        <td className="as-col-material">
          <div style={{ color: "#2e343a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={getGenFgCode(order, pmxSplitRules) || ""}>{getGenFgCode(order, pmxSplitRules) || "—"}</div>
          {order.kb_sfg_material_code && <div className="as-sub-text">{order.kb_sfg_material_code}</div>}
        </td>
...
/* ── Main modal ── */
export default function FeedmillAutoSequenceModal({ data, isLoading = false, changeoverRules, pmxSplitRules = [], onApply, onCancel }) {
  const [selectedOrders, setSelectedOrders] = useState(() =>
    new Set((data?.selectedToMove || []).filter(c => c._isWorthMoving).map(c => c.id))
  );
  // orderedResults[line] = manually-reordered result for that line (overrides computed)
  const [orderedResults, setOrderedResults] = useState({});
  // combinedResults[line] = result after combine was run for that line
  const [combinedLines, setCombinedLines] = useState(new Set());
  ...
  const [activeLineTab, setActiveLineTab] = useState(data?.feedmillLines?.[0] || "");

  // Reseed when data changes
  useEffect(() => {
    if (!data) return;
    setSelectedOrders(new Set((data.selectedToMove || []).filter(c => c._isWorthMoving).map(c => c.id)));
    setOrderedResults({});
    setCombinedLines(new Set());
    setExtraPlacementLog([]);
    setAiExplanations(null);
    setUseFallback(false);
    setActiveLineTab(data.feedmillLines?.[0] || "");
  }, [data]);

  // Build after-sequence for every feedmill line based on selected candidates
  const sequencedByLine = useMemo(() => {
    if (!data) return {};
    const result = {};
    for (const line of data.feedmillLines) {
      // Base: current orders on this line
      const lineOrders = (data.originalSnapshot[line] || []).filter(o => !o.parent_id).map(o => ({ ...o }));
      // Add selected candidates that target this line
      const moved = (data.selectedToMove || [])
        .filter(c => selectedOrders.has(c.id) && c._targetLine === line)
        .map(c => ({
          ...c,
          feedmill_line: line,
          _movedFromLine: c._sourceLineNormalized || c._sourceLine,
          _movement: "new_to_line",
        }));
      const combined = [...lineOrders, ...moved];
      const sorted = localSort(combined);
      const withCO = applyPreviewChangeovers([...sorted], changeoverRules);
      const originalIds = new Set(lineOrders.map(o => o.id));
      result[line] = withCO.map((order, index) => {
        const newPrio = index + 1;
        if (!originalIds.has(order.id)) return { ...order, prio: newPrio, _movement: "new_to_line" };
        const origIdx = lineOrders.findIndex(o => o.id === order.id);
  ...
  // Display = manual order override OR computed
  function getDisplayResult(line) {
    return orderedResults[line] || sequencedByLine[line] || [];
  }

  // Combined placement log
  const placementLog = useMemo(() => {
    if (!data) return [];
    const moveMoved = (data.selectedToMove || [])
      ...
      .map(order => ({
        ...
        toLine: order._targetLine, volume: parseFloat(order.total_volume_mt) || 0, fpr: order.fpr,
        lineScores: [
          ...
          { line: order._targetLine, runRate: order._targetRunRate || 10, totalMTBefore: order._targetMTBefore || 0, queueTimeBefore: order._targetQueueBefore || 0, totalMTAfter: order._targetMTAfter || 0, queueTimeAfter: order._targetQueueAfter || 0 },
        ],
        bestLineReason: { line: order._targetLine, runRate: order._targetRunRate || 10, queueTime: order._targetQueueBefore || 0, totalMTBefore: order._targetMTBefore || 0, totalMTAfter: order._targetMTAfter || 0 },
  ...
  function selectAll() {
    ...
    setOrderedResults({}); setCombinedLines(new Set()); setExtraPlacementLog([]); setAiExplanations(null);
  }
  ...
  async function handleCombine() {
    if (isCombining || !data) return;
    setIsCombining(true);
    const newOrderedResults = { ...orderedResults };
    const newExtraLog = [...extraPlacementLog];
    for (const line of data.feedmillLines) {
      if (combinedLines.has(line)) continue;
      const current = getDisplayResult(line).map(o => ({ ...o, feedmill_line: line }));
      const { combined, placementLog: combineLog } = combineOnLine(current, line);
      const resorted = localSort(combined);
      const withCO = applyPreviewChangeovers([...resorted], changeoverRules);
      const origLineOrders = (data.originalSnapshot[line] || []).filter(o => !o.parent_id);
      const originalIds = new Set(origLineOrders.map(o => o.id));
      newOrderedResults[line] = withCO.map((order, index) => ({
        ...order, prio: index + 1,
        _movement: !originalIds.has(order.id) && !order._isCombined ? "new_to_line" : order._movement || "same",
      }));
      newExtraLog.push(...combineLog);
    }
    setOrderedResults(newOrderedResults);
    setExtraPlacementLog(newExtraLog);
    setCombinedLines(new Set(data.feedmillLines));
    setIsCombining(false);
    setAiExplanations(null);
    setUseFallback(false);
  }

  function onDragEnd(result, line) {
    if (!result.destination || result.destination.index === result.source.index) return;
    const current = [...getDisplayResult(line)];
    const [moved] = current.splice(result.source.index, 1);
    current.splice(result.destination.index, 0, moved);
    setOrderedResults(prev => ({ ...prev, [line]: current.map((o, i) => ({ ...o, prio: i + 1 })) }));
  }

  const candidateCount = data ? (data.selectedToMove || []).length : 0;
  const isCombined = combinedLines.size > 0;

  // Total after counts
  const totalAfterCount = data
    ? data.feedmillLines.reduce((s, l) => s + (getDisplayResult(l).length || 0), 0)
    : 0;
  const totalMovedCount = selectedOrders.size;

  // Per-line stats for tab badges
  function lineTabStats(line) {
    const disp = getDisplayResult(line);
    const changes = disp.filter(o => o._movement !== "same").length;
    return { count: disp.length, changes };
  }

  // Active line data
  const activeLine = activeLineTab || data?.feedmillLines?.[0] || "";
  const activeBeforeOrders = useMemo(() => {
    if (!data) return [];
    const rows = (data.originalSnapshot[activeLine] || []).filter(o => !o.parent_id).map(o => ({ ...o }));
    applyPreviewChangeovers(rows, changeoverRules);
    return rows;
  }, [data, activeLine, changeoverRules]);
  const activeAfterOrders = getDisplayResult(activeLine);
  const activeMovedCount = activeAfterOrders.filter(o => o._movement === "new_to_line").length;
  const activeCombinedCount = activeAfterOrders.filter(o => o._isCombined).length;

  return (
    <div className="plant-modal-overlay nexfeed-force-light" style={{ zIndex: 10100 }} onClick={e => e.stopPropagation()}>
      <div className="plant-modal-container" style={{ maxWidth: 1440 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="plant-modal-header">
          <div>
            <h2 className="plant-modal-title">
              ⚙️ Optimize {getFeedmillFullName(data?.feedmillShortName)}
            </h2>
            <div className="plant-modal-subtitle">
              Find orders from other lines that can move to {getFeedmillFullName(data?.feedmillShortName)} ({(data?.feedmillLines || []).map(getLineShortName).join(" & ")})
            </div>
          </div>
        ...
        {isLoading ? (
          <div className="flex-1 flex flex-col items-center justify-center py-16 px-8">
            ...
            <p className="text-sm font-medium text-[#2e343a]">Analyzing orders…</p>
          ...
          <>
            <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
              ...
              <div className="plant-stats-strip">
                <StatBadge label="Current Orders" value={(data.feedmillLines || []).reduce((s, l) => s + (data.originalSnapshot[l] || []).filter(o => !o.parent_id).length, 0)} />
                ...
                <StatBadge label="After Optimization" value={totalAfterCount} />
                {isCombined && <StatBadge label="Combined" value={(data.feedmillLines || []).reduce((s, l) => s + getDisplayResult(l).filter(o => o._isCombined).length, 0)} color="green" />}
              </div>
              ...
              {candidateCount > 0 ? (
                <div className="line-as-candidates">
                  <div className="line-as-candidates-header">
                    <span className="line-as-candidates-title">
                      📋 Orders available to move to {getFeedmillFullName(data.feedmillShortName)}
                    </span>
                    <div className="line-as-candidates-actions">
                      <button className="line-as-select-btn" onClick={selectAll} disabled={isApplying}>Select All</button>
                      <button className="line-as-select-btn" onClick={deselectAll} disabled={isApplying}>Deselect All</button>
                    </div>
                  </div>
                  <div className="line-as-candidates-list">
                    {(data.selectedToMove || []).map((candidate) => (
                      <label key={candidate.id} className="line-as-candidate-row" style={{ cursor: isApplying ? "default" : "pointer" }}>
                        <input
                          type="checkbox"
                        ...
                        {/* Target line */}
                        <span style={{ fontSize: 11, color: "#16a34a", fontWeight: 600, marginLeft: 4, flexShrink: 0 }}>
                          → {getLineShortName(candidate._targetLine || "")}
                        </span>
                ...
                <div className="line-as-no-candidates">
                  No orders from other lines can be moved to {data.feedmillShortName} at this time. All eligible lines have lower or equal queue times.
                </div>
              )}

              {/* Line tabs within feedmill */}
              <div style={{ display: "flex", gap: 4, padding: "10px 20px 0", borderTop: "1px solid #f3f4f6", marginTop: 8 }}>
                {(data.feedmillLines || []).map(line => {
                  const { count, changes } = lineTabStats(line);
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        cursor: "pointer", border: "1px solid #e5e7eb", borderBottom: isActive ? "1px solid #fff" : "1px solid #e5e7eb",
                        background: isActive ? "var(--color-bg-secondary)" : "var(--color-bg-tertiary)",
                        color: isActive ? "var(--nexfeed-primary)" : "#6b7280",
                        marginBottom: isActive ? -1 : 0,
                        display: "flex", alignItems: "center", gap: 6,
                      }}
                    >
                      {getLineShortName(line)}
                      <span style={{
                        fontSize: 10, fontWeight: 700, borderRadius: 999, padding: "1px 6px",
                        background: isActive ? "var(--nexfeed-primary)" : "#e5e7eb",
                        color: isActive ? "#fff" : "#6b7280",
                      }}>{count}</span>
                      {changes > 0 && (
                        <span style={{
                          fontSize: 10, fontWeight: 700, borderRadius: 999, padding: "1px 5px",
              ...
              {/* Before / After for active line */}
              <div className="plant-line-comparison" style={{ minHeight: 300, marginTop: 0, borderRadius: "0 8px 8px 8px" }}>
                  ...
                  <div className="plant-line-comparison-scroll">
                    ...
                    <div className="as-before-panel">
                      <div className="as-before-label">
                        <span>📋</span> Before — {activeBeforeOrders.length} orders on {getLineShortName(activeLine)}
                      </div>
                      <div className="as-before-table-wrap">
                        <table className="as-before-table" style={{ minWidth: 1100 }}>
                          ...
                          <tbody>
                            {activeBeforeOrders.map((order, idx) => (
                            ...
                            {activeBeforeOrders.length === 0 && (
                              <tr><td colSpan={14} style={{ textAlign: "center", color: "#9ca3af", padding: "32px 8px", fontSize: 12, fontStyle: "italic" }}>No orders on {getLineShortName(activeLine)} before sequencing</td></tr>
                            )}
                          </tbody>
                        </table>
                      </div>
                    ...
                    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                      <div className="as-after-label">
                        <span>✨</span> After — {activeAfterOrders.length} orders on {getLineShortName(activeLine)}
                        {activeMovedCount > 0 && ` · ${activeMovedCount} from other lines`}
                        {activeCombinedCount > 0 && ` · ${activeCombinedCount} combined`}
                      </div>
                      <div style={{ flex: 1, overflowY: "auto", overflowX: "auto" }}>
                        <DragDropContext onDragEnd={r => onDragEnd(r, activeLine)}>
                          <table className="as-after-table" style={{ minWidth: 1100 }}>
                            ...
                            <Droppable droppableId={`fm-after-${activeLine}`}>
                              {(droppableProvided) => (
                                <tbody ref={droppableProvided.innerRef} {...droppableProvided.droppableProps}>
                                  {activeAfterOrders.map((order, index) => (
                                    <Draggable
              ...
              {placementLog.length > 0 && (
                <div className="line-actions-plain">
                  ...
                  {!isLoadingAI && placementLog.map((entry, index) => {
                    ...
                    const showMTBadge = toScore && entry.type !== "combined";
                          ...
                          {entry.type === "combined" ? (
                            <>
                              ...
                              <span className="plant-action-title">
                                Combined <strong>{entry.ordersCount}</strong> orders of <strong>{entry.product}</strong> ({parseFloat(entry.totalVolume || 0).toFixed(1)} MT) on {entry.toLine}
                              </span>
                            </>
                          ) : (
                            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setIsApplying(true);
                    // Build sequencedByLine from display results
                    const seqByLine = {};
                    (data.feedmillLines || []).forEach(line => {
                      seqByLine[line] = getDisplayResult(line);
                    });
                    const affectedSourceLines = [...new Set(
                      (data.selectedToMove || [])
                        .filter(o => selectedOrders.has(o.id))
                        .map(o => o._sourceLineNormalized || o._sourceLine)
                        .filter(Boolean)
                    )];
                    onApply({ feedmillLines: data.feedmillLines, sequencedByLine: seqByLine, affectedSourceLines })
                      .catch(() => {})
