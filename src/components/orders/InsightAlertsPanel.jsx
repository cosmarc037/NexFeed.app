...
export default function InsightAlertsPanel({ orders, lastUploadDate, lastN10DUploadDate = null, inferredTargetMap = {}, onUpload, onScrollToOrder, onAutoSequence = null, onNavigateToN10D = null }) {
  const [tickState, setTickState] = useState(0);
  const timerRef = useRef(null);
  const [insightsOpen, setInsightsOpen] = useState(() => readLS('nexfeed_insights_open', true));
  const [alertsOpen, setAlertsOpen] = useState(() => readLS('nexfeed_alerts_open', true));
        ...
        >
          ...
          <div className={`panel-body-collapse${insightsOpen ? '' : ' collapsed'}`}>
            <div className="panel-body-inner">
              <div style={{ background: '#fafafa', padding: 12, display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 350, overflowY: 'auto', borderRadius: '0 0 8px 8px' }}>
                {insightCount === 0 ? (
