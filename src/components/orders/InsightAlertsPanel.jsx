...
function OrderRef({ fpr, name, orderId, onScrollToOrder }) {
  ...
  return (
    <span
      ...
      onMouseLeave={e => { e.currentTarget.style.color = 'var(--color-text)'; e.currentTarget.style.textDecoration = 'underline dotted #d1d5db'; }}
      onClick={handleClick}
...
function InsightCard({ emoji, title, children }) {
  return (
    ...
    >
      <p style={{ fontWeight: 500, fontSize: 12, color: 'var(--color-text)', marginBottom: 4 }}><span style={{ filter: 'grayscale(1)', opacity: 0.7 }}>{emoji}</span> {title}</p>
...
function formatSummaryContent(text) {
  ...
  return lines.map((line, i) => {
    ...
    if (numbered) {
      return (
        <div key={i} style={{ display: 'flex', gap: 6, fontSize: 12, color: 'var(--color-text)', lineHeight: 1.6, padding: '2px 0' }}>
          <span style={{ fontWeight: 700, color: 'var(--color-text)', minWidth: 16, flexShrink: 0 }}>{numbered[1]}.</span>
    ...
    if (isHeader) {
      return <div key={i} style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text)', margin: '10px 0 6px 0' }}>{line}</div>;
    }
    return <p key={i} style={{ margin: '0 0 8px 0', fontSize: 12, color: 'var(--color-text)', lineHeight: 1.6 }}>{line}</p>;
  });
}
...
function AISummaryCard({ type, orders, feedmill, line, inferredTargetMap, isOpen, lineShutdowns = {}, kbRecords = [] }) {
  ...
  return (
    <div className="rounded-md" style={{ background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '10px 12px' }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--color-text-muted)', lineHeight: 1.5, minHeight: 20 }}>
        {bodyContent}
      </div>
    </div>
  );
...
function computeInsights(orders, onScrollToOrder, inferredTargetMap = {}, onAutoSequence = null) {
  const now = new Date();
  const categoryBuckets = { n10d: [], priority: [], cut: [], sequence: [], capacity: [] };
  console.debug('[Production Insights Filter]', {
    removedInsightType: 'consider_combining',
    appliedToTabs: [
      'all_feedmills',
      'feedmill_1',
      'feedmill_2',
      'feedmill_3',
      'powermix',
      'fulfillment_demo_all_feedmills',
    ],
    productionInsightsStillActive: true,
  });
  const active = orders.filter(o => o.status !== 'completed' && o.status !== 'cancel_po');
  ...
  const catOrder = ['n10d', 'priority', 'cut', 'sequence', 'capacity'];
  const result = [];
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row gap-4" data-testid="section-insight-alerts">
        ...
        >
          ...
          >
            <span style={{ fontWeight: 500, fontSize: 13, color: 'var(--color-text)' }}>
              <span style={{ filter: 'grayscale(1)', opacity: 0.7 }}>🧠</span> Production Insights
              {!insightsOpen && (
        ...
        >
          ...
          >
            <span style={{ fontWeight: 500, fontSize: 13, color: 'var(--color-text)' }}>
              <span style={{ filter: 'grayscale(1)', opacity: 0.7 }}>🔔</span> Alerts & Reminders
              {!alertsOpen && (
