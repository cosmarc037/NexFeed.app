...
import { applyPreviewChangeovers, generateFallbackExplanation, getLineRunRate, getOrderVolumeDisplayState } from "./PlantAutoSequenceModal";
...
/* ── After-row with DnD ── */
function LineAfterRow({ order, prio, provided, snapshot, destinationLine, pmxSplitRules = [] }) {
    ...
    <>
      ...
      >
        ...
        <td className="as-col-material">
          ...
          {order.kb_sfg_material_code && <div style={{ color: "#2e343a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{order.kb_sfg_material_code}</div>}
        </td>
        ...
        <td className="as-col-prod">
          {(() => {
            const _lvol = getOrderVolumeDisplayState(order).displayVolume;
            const _lrate = order._displayRunRate ?? getEffectiveRunRate(order);
            const _lisManual = order.production_hours_manual === true || (order.form || '').trim().toUpperCase() === 'M';
            const _lpt = (_lisManual && parseFloat(order.production_hours) > 0)
              ? parseFloat(order.production_hours)
              : (_lrate > 0 ? _lvol / _lrate : 0);
            console.debug('[Auto-Sequence Preview Production Time From Displayed Rate]', {
              ...
              orderId: order.id,
              volume: _lvol,
              displayedRate: _lrate,
              expectedProductionTime: _lrate > 0 ? _lvol / _lrate : null,
              previewProductionTime: _lpt,
              matchesExpected: _lrate > 0 ? Math.abs(_lpt - (_lvol / _lrate)) < 0.01 : false,
            });
            return <>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#1a1a1a" }}>
                {_lpt > 0 ? `${formatProductionTime(_lpt)} hours` : "—"}
              </div>
              <div className="as-sub-text">CO: {formatProductionTime(order._changeoverTotal || 0)}</div>
              <div className="as-sub-text">Rate: {_lrate > 0 ? _lrate.toFixed(2) : "—"}</div>
            </>;
          })()}
        </td>
...
/* ── Main modal ── */
export default function LineAutoSequenceModal({ data, isLoading = false, inferredTargetMap = {}, changeoverRules, pmxSplitRules = [], onApply, onCancel }) {
  ...
  const sequencedResult = useMemo(() => {
    ...
    const result = withCO.map((order, index) => {
    ...
    result.forEach((order) => {
      const displayedRate = order._displayRunRate ?? getEffectiveRunRate(order);
      const rowRR = parseFloat(order.run_rate);
      const usedGenericFallback = !(Number.isFinite(rowRR) && rowRR > 0);
      const coValue = parseFloat(order._changeoverTotal || 0);
      console.debug('[Auto-Sequence Preview Rate Display]', {
        line: order.feedmill_line || data.targetLine, feedmill: null,
        orderId: order.id, previewProductionTime: order.production_hours,
        coValue, displayedRate, displayFormat: ['production_time', 'CO', 'Rate'],
      });
      console.debug('[Auto-Sequence Preview Production Time Basis]', {
        line: order.feedmill_line || data.targetLine, feedmill: null,
        orderId: order.id, volume: parseFloat(order.total_volume_mt) || 0,
        displayedRate,
        expectedProductionTime: displayedRate > 0 ? (parseFloat(order.total_volume_mt) || 0) / displayedRate : null,
        previewProductionTime: order.production_hours,
        usesDisplayedRateForCalculation: false,
      });
        ...
        <div className="plant-modal-header">
          <div>
            <h2 className="plant-modal-title">
              <span style={{ filter: 'grayscale(1)', opacity: 0.7 }}>⚙️</span> Optimize {data ? getLineDisplayName(data.targetLineLabel) : "Loading…"}
            </h2>
              ...
              <div className="line-as-candidates">
                <div className="line-as-candidates-header">
                  <span className="line-as-candidates-title">
                    <span style={{ filter: 'grayscale(1)', opacity: 0.7 }}>📋</span> Orders available to move to {getLineDisplayName(data.targetLineLabel)}
                  </span>
            ...
            {placementLog.length > 0 && (
              <div className="line-actions-plain">
                <div className="plant-actions-header">
                  <div className="plant-summary-title"><span style={{ filter: 'grayscale(1)', opacity: 0.7 }}>📋</span> Actions Taken</div>
                  {!isLoadingAI && (
                  ...
                  return (
                    <div key={index} className="plant-action-card">
                      <div className="plant-action-header">
                        {entry.type === "combined" ? (
                          <>
                            <span className="plant-action-icon plant-action-combine" style={{ filter: 'grayscale(1)', opacity: 0.7 }}>🔗</span>
