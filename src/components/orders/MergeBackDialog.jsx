...
export default function MergeBackDialog({ open, onClose, onConfirm, portion1, portion2 }) {
  ...
  const orderId = portion1.id;

  const handleConfirm = async () => {
    if (isConfirming) return;
    console.debug('[Order Action Modal Confirm Click]', { modalType: 'merge_back', orderId, loadingStarted: true });
    setIsConfirming(true);
    console.debug('[Order Action Modal Loading State]', { modalType: 'merge_back', orderId, confirmButtonDisabled: true, loadingVisible: true });
    let success = false;
    try {
      await onConfirm();
      success = true;
    } catch (err) {
      console.error('[MergeBackDialog] confirm error', err);
      setIsConfirming(false);
    } finally {
      console.debug('[Order Action Modal Processing Complete]', { modalType: 'merge_back', orderId, success, loadingEnded: true });
    }
  };

  return (
    <Dialog open={open} onOpenChange={(o) => !o && !isConfirming && onClose()}>
      <DialogContent className="merge-back-dialog max-w-[480px]" data-testid="dialog-merge-back">
        <DialogHeader>
        ...
        <DialogFooter className="mt-4">
          ...
          <Button
            ...
            disabled={isConfirming}
            className="bg-[var(--nexfeed-primary)] hover:bg-[var(--nexfeed-primary-dark)] text-white disabled:opacity-70 disabled:cursor-not-allowed text-[14px] font-semibold h-10 px-5"
