...
export default function Next10DaysManager({
  onApplied,
  sapOrders = [],
  onUpdateOrder,
  recordEntity = null,
  uploadEntity = null,
  recordsQueryKey = "n10d_records",
  uploadsQueryKey = "n10d_uploads",
  showAdjustedInventory = false,
  adjustedInventoryMap = {},
}) {
  const RecordEntity = recordEntity || Next10DaysRecord;
  const UploadEntity = uploadEntity || Next10DaysUpload;
  ...
  const { data: uploads = [] } = useQuery({
    ...
    queryFn: () => UploadEntity.list("-created_date", 10),
    staleTime: 0,
  });
  ...
  const { data: allRecords = [] } = useQuery({
    ...
    queryFn: () => RecordEntity.list("-created_date", 2000),
    staleTime: 0,
  });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(
        sessionRecords.map((r) => RecordEntity.delete(r.id)),
      );
      // Delete the upload metadata
      await UploadEntity.delete(upload.id);
      queryClient.invalidateQueries({ queryKey: [recordsQueryKey] });
      queryClient.invalidateQueries({ queryKey: [uploadsQueryKey] });
  ...
  // ── Volume classification (demand magnitude) ───────────────────────────────
  // Pre-computed across ALL sortedRecords so percentile ranks are dataset-wide.
  // Score = DFL + sum of 10-day daily demand (mirrors "total demand burden").
  const _volumeClassMap = (() => {
    const scores = sortedRecords.map(r => {
      const dv = parseDailyValuesArr(r.daily_values);
      const dfl = r.due_for_loading || 0;
      const next10 = dv.reduce((s, d) => s + (parseFloat(d.value) || 0), 0);
      return { key: String(r.material_code || r.id || ''), score: dfl + next10, dfl, next10 };
    });
    const sorted = [...scores].sort((a, b) => b.score - a.score);
    const n = sorted.length;
    const map = new Map();
    scores.forEach(({ key, score, dfl, next10 }) => {
      const rank = sorted.findIndex(x => x.key === key) + 1;
      const pctRank = n > 0 ? rank / n : 1;
      let cls;
      if (score <= 0)        cls = 'Low';
      else if (pctRank <= 0.20) cls = 'High';
      else if (pctRank <= 0.50) cls = 'Moderate';
      else                   cls = 'Low';
      const percentile = Math.round((1 - (rank - 1) / Math.max(n, 1)) * 100);
      map.set(key, { cls, score, percentile, dfl, next10DayTotal: next10 });
    });
    return map;
  })();

  // One-time log for column presence
  console.debug('[Future Dispatches New Columns]', {
    columnsAddedAfterStatus: ['Volume', 'Velocity'],
    volumeUsesTooltip: true,
    velocityUsesTooltip: true,
  });
  ...
  const frozenSubTh = (leftPx, isLast = false) => ({
    ...
    borderRight: isLast ? "1px solid var(--color-border)" : undefined,
  });
    ...
    <Card className="border-0 shadow-sm">
      ...
      <CardContent className="space-y-5">
        ...
        {sortedRecords.length > 0 && (
          <div
            ref={tableScrollRef}
            className="overflow-auto rounded-lg border border-gray-100 n10d-table-light"
            ...
            >
              <thead>
                ...
                <tr ref={groupHeaderRef}>
                  ...
                  {/* TRACKING — spans Inventory, Bal.to.Prod, Completion, Avail, Status, Volume, Velocity */}
                  <th
                    colSpan={showAdjustedInventory ? 8 : 7}
                    data-tour="future-dispatches-header-tracking"
                    style={{ ...groupThBase, borderLeft: "1px solid var(--color-border)" }}
                  >
                    Tracking
                  </th>
                </tr>
                ...
                <tr>
                  ...
                  {/* Adjusted Inventory — demo only */}
                  {showAdjustedInventory && (
                    <th
                      style={{
                        ...subThBase,
                        textAlign: "center",
                      ...
                      data-testid="header-adjusted-inventory"
                    >
                      Adj. Inventory
                    </th>
                  )}
                  ...
                  <th
                    ...
                    title="Demand magnitude: High = top 20%, Moderate = next 30%, Low = remaining 50% (based on DFL + 10-day forecast total)"
                  >
                    Volume
                  </th>
                  {/* Velocity */}
                  <th
                    ...
                    title="Demand stability: Stable (CV ≤ 0.5), Less Stable (CV 0.5–1.0), Erratic (CV > 1.0) — based on 10-day daily demand variability"
                  >
                    Velocity
                  </th>
                </tr>
              ...
              <tbody>
                {displayRecords.map((rec, idx) => {
                  ...
                  // ── Volume classification (from pre-computed dataset-wide map) ──
                  const _matKey = String(rec.material_code || rec.id || '');
                  const _volMeta = _volumeClassMap.get(_matKey) || { cls: 'Low', score: 0, percentile: 0, next10DayTotal: 0 };
                  const volumeClass = _volMeta.cls;

                  // ── Velocity classification (coefficient of variation of daily demand) ──
                  const _dailyVals = dateColumnsForTable.map(d => dvMap[d] || 0);
                  const _vN = _dailyVals.length;
                  const _vMean = _vN > 0 ? _dailyVals.reduce((s, v) => s + v, 0) / _vN : 0;
                  const _vStd = _vN > 1
                    ? Math.sqrt(_dailyVals.reduce((s, v) => s + (v - _vMean) ** 2, 0) / _vN)
                    : 0;
                  const _vCV = _vMean > 0.001 ? _vStd / _vMean : (_vStd > 0 ? 999 : 0);
                  const velocityClass = _vCV <= 0.5 ? 'Stable' : _vCV <= 1.0 ? 'Less Stable' : 'Erratic';

                  console.debug('[Future Dispatches Volume Classification]', {
                    materialCode: rec.material_code,
                    dfl,
                    next10DayDemandTotal: _volMeta.next10DayTotal,
                    volumeScore: _volMeta.score,
                    percentileRank: _volMeta.percentile,
                    volumeClass,
                  });
                  console.debug('[Future Dispatches Velocity Classification]', {
                    materialCode: rec.material_code,
                    dailyValues: _dailyVals,
                    meanDemand: _vMean,
                    stdDevDemand: _vStd,
                    coefficientOfVariation: _vCV === 999 ? null : _vCV,
                    velocityClass,
                  });
                  ...
                  // Adjusted Inventory (demo only) — folded into the SAME product
                  // tooltip below, not a separate hover card.
                  const _adjMatKey = String(
                    rec.material_code || rec.material_code_fg || "",
                  ).trim();
                  const _adj = showAdjustedInventory
                    ? adjustedInventoryMap[_adjMatKey]
                    : null;
                  const adjustedInventory =
                    _adj && typeof _adj.value === "number"
                      ? {
                          value: _adj.value,
                          inventory: inv,
                          satisfied: _adj.satisfied || 0,
                          avgDaily: _adj.avgDaily || 0,
                        }
                      : null;
                  if (showAdjustedInventory) {
                    console.debug(
                      "[Demo Future Dispatches Tooltip - Adjusted Inventory]",
                      {
                        product: rec.item_description || "—",
                        inventory: inv,
                        satisfiedOrderVolume: adjustedInventory?.satisfied ?? 0,
                        averageDaily: adjustedInventory?.avgDaily ?? 0,
                        adjustedInventory: adjustedInventory?.value ?? null,
                        embeddedInExistingTooltip: true,
                      },
                    );
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={dueRed ? "n10d-red-cell" : ""}
                        style={{
                          ...frozenTdBase,
                          left: LEFT.dfl,
                          minWidth: COL_W.dfl,
                          ...
                          ...(dueRed
                            ? redStyle
                            : { color: "var(--color-text)", background: frozenBg }),
                        }}
                      >
                        {dfl.toFixed(1)}
                      </td>
                      ...
                      {showAdjustedInventory && (() => {
                        ...
                        const adj = adjustedInventoryMap[matKey];
                        const hasAdj = adj && typeof adj.value === "number";
                        // No standalone title tooltip here — the Adjusted Inventory
                        // value + formula is folded into the shared product tooltip
                        // (ProductTooltipPanel) shown when hovering this row.
                        return (
                          <td
                            data-testid={`cell-adjusted-inventory-${matKey}`}
                            style={{
                              padding: cellPad,
                              textAlign: "center",
                              fontWeight: 700,
                      ...
                      <td
                        className={balVal !== "" && parseFloat(balVal) < 0 ? "n10d-deficit-cell" : ""}
                        style={{
                          padding: cellPad,
                          textAlign: "center",
                          minWidth: 68,
                          background: isHovered ? hoverBg : undefined,
                          color: balVal !== "" && parseFloat(balVal) < 0 ? "#dc2626" : "var(--color-text)",
                      ...
                      {/* Volume — demand magnitude cell highlight */}
                      <td
                        data-testid={`cell-volume-${rec.material_code}`}
                        title={`Total demand score: ${(dfl + _volMeta.next10DayTotal).toFixed(1)} MT (DFL ${dfl.toFixed(1)} + 10-day forecast ${_volMeta.next10DayTotal.toFixed(1)}) → ${volumeClass}`}
                        ...
                        style={{
                          ...
                          cursor: "help",
                          ...(volumeClass === "High"
                            ? { background: isHovered ? "#fca5a5" : "#fee2e2", color: "#b91c1c" }
                            : volumeClass === "Moderate"
                            ? { background: isHovered ? "#fcd34d" : "#fef3c7", color: "#92400e" }
                            : { background: isHovered ? "#93c5fd" : "#dbeafe", color: "#1d4ed8" }),
                        }}
                      >
                        {volumeClass}
                      </td>

                      {/* Velocity — demand stability cell highlight */}
                      <td
                        data-testid={`cell-velocity-${rec.material_code}`}
                        title={`CV = ${_vCV.toFixed(2)} (std ${_vStd.toFixed(1)} ÷ mean ${_vMean.toFixed(1)} MT/day) — ${velocityClass}`}
                        onMouseOver={e => e.stopPropagation()}
                        style={{
                          padding: cellPad,
                          textAlign: "center",
                          ...
                          ...(velocityClass === "Stable"
                            ? { background: isHovered ? "#86efac" : "#dcfce7", color: "#15803d" }
                            : velocityClass === "Less Stable"
                            ? { background: isHovered ? "#fcd34d" : "#fef3c7", color: "#92400e" }
                            : { background: isHovered ? "#fca5a5" : "#fee2e2", color: "#b91c1c" }),
                        }}
                      >
            ...
            (() => {
              ...
              const inputBorderColor = isExact
                ...
                : isWrong
                  ...
                  : "#d1d5db";
              return (
                <div className="n10d-delete-dialog">
                  {/* Title */}
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <p
                        className="n10d-del-warning-text"
                        style={{
                          fontSize: 11,
                          color: "#991b1b",
                          lineHeight: 1.5,
                  ...
                  <div style={{ marginBottom: 18 }}>
                    <label
                      className="n10d-del-confirm-label"
                      style={{
                        display: "block",
                        fontSize: 12,
                        fontWeight: 500,
