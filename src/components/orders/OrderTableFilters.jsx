...
export const COLUMN_FILTER_CONFIG = {
  ...
  cost:   { type: "number-range", label: "Cost" },
  margin: { type: "number-range", label: "Margin (%)" },
};
...
function _getColValue(order, colKey) {
  switch (colKey) {
    ...
    case "cost":   return parseFloat(order._pricing_cost)   ?? null;
    case "margin": return parseFloat(order._pricing_margin) ?? null;
    default: return "";
  }
}
...
export function applyColumnFilters(orders, activeFilters) {
  ...
  return orders.filter((order) => {
    for (const [colKey, filter] of Object.entries(activeFilters)) {
      ...
      if (colKey === "cost" || colKey === "margin") {
        console.debug("[Pricing Filter Applied]", {
          table: "order-table",
          column: colKey,
