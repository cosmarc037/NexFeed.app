...
// Sum effective volume (volume_override takes priority) for a list of active orders
function getRunningBalanceMT(activeOrders) {
  return (activeOrders || []).reduce((sum, o) => {
    const vol =
      o.volume_override != null && o.volume_override !== ""
        ? parseFloat(o.volume_override) || 0
        : parseFloat(o.total_volume_mt) || 0;
    return sum + vol;
  }, 0);
}

function RunningBalanceBadge({ activeOrders }) {
  const total = getRunningBalanceMT(activeOrders);
  return (
    <span
      data-testid="badge-running-balance"
      title="Sum of Volume (MT) for all non-Done orders on this line"
      style={{
        display: "inline-flex",
        alignItems: "center",
        gap: 4,
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-end gap-2 flex-wrap">
          ...
          <Button
            ...
            className="bg-[var(--nexfeed-primary)] hover:bg-[var(--nexfeed-primary-dark)] h-9 text-[12px] text-white"
            size="sm"
        ...
        <div className="flex items-center justify-between gap-4" data-tour="orders-search">
          ...
          {(allFmPerLineData || []).map(({ feedmill, fmLabel, line, completed, active, cancelled, unfilteredActive }) => {
            ...
            return (
              <div key={line} data-testid={`section-line-${line}`}>
                <div className={`fm-line-header-bar flex items-center justify-between px-4 py-2.5 ${headerBgCls} rounded-t-lg`} style={{ borderBottom: isLineDown ? "1px solid #f87171" : isDark ? "1px solid #374151" : "1px solid #d1d5db" }}>
                    ...
                    {!isLineDown && onAddOrder && (
                      <>
                        ...
                        <button
                          ...
                          style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 12, fontWeight: 500, background: "none", border: "none", padding: 0, cursor: "pointer" }}
                  ...
                  <div className="flex items-center gap-2">
                    <RunningBalanceBadge activeOrders={active} />
                ...
                <div className={`fm-line-container border border-t-0 ${containerBorderCls} rounded-b-lg overflow-hidden`}>
                  {isLineDown && (
                  ...
                  {noActiveMatch ? (
                    ...
                    <OrderTable
                      ...
                      onMashShutdownDivertOrder={onMashShutdownDivertOrder}
                      onRevertOrder={onRevertOrder}
                      onNavigateToPmxLinked={onNavigateToPmxLinked}
                      pmxSourceToGeneratedMap={pmxSourceToGeneratedMap}
                      pendingPreorders={pendingPreorders}
                      onApprovePreorder={onApprovePreorder}
                      onDismissPreorder={onDismissPreorder}
                      onCancelReorder={onCancelReorder}
                      onRefreshAiPlacement={onRefreshAiPlacement}
                      hideDivertRevert={hideDivertRevert}
                      workspace={workspace}
                    />
                  )}
                </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2">
          {onAutoSequence && activeFeedmill !== "PMX" && (() => {
      ...
      {isAllTab && perLineData ? (
        ...
        (() => {
          const resolvedSingleLine = (activeSubSection && activeSubSection !== "all")
            ? activeSubSection
            : (fmLines.length === 1 ? fmLines[0] : null);
          const singleLineShutdown = resolvedSingleLine ? lineShutdowns[resolvedSingleLine] : null;
          const isSingleLineDown = !!(singleLineShutdown?.isShutdown);
          return (
            <div className={`fm-line-container ${isSingleLineDown ? "border border-red-400" : "border border-gray-200"} rounded-lg overflow-hidden`}>
              {isSingleLineDown && (
                <div style={{ background: "#fef2f2", color: "#dc2626", fontSize: "12px", fontWeight: 600, padding: "8px 16px", borderBottom: "1px solid #fecaca" }}>
                  🔴 {resolvedSingleLine} — SHUTDOWN{singleLineShutdown.since ? ` since ${new Date(singleLineShutdown.since).toLocaleDateString()}` : ""}
                  {singleLineShutdown.reason ? ` · ${singleLineShutdown.reason}` : ""}
                </div>
              )}
              {!isSingleLineDown && (
                <div
                  className="fm-line-header-bar bg-gray-200"
                  style={{
                ...
                >
                  ...
                  <RunningBalanceBadge activeOrders={filteredOrders.active} />
                </div>
              )}
              <OrderTable
                orders={[
