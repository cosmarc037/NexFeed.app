...
import { callPlantActionsAI, buildPlantActionsPrompt, parsePlantActionsResponse, generatePlantRowSequenceInsights } from "@/services/azureAI";
import {
...
import { calculateChangeoverBetween } from "@/utils/changeoverCalc";
...
function getRowStatus(order, inferredTargetMap) {
  ...
  const inf = inferredTargetMap?.[order.material_code] || inferredTargetMap?.[order.material_code_fg];
  if (!inf) return "grey";
  if (inf.status === "Critical") return "red";
  if (inf.status === "Urgent") return "amber";
  if (inf.status === "Monitor") return "blue";
  if (inf.status === "Sufficient") return "lightgrey";
  return "grey";
}
...
function formatProductionHours(h) {
  if (!h || isNaN(parseFloat(h))) return "0.00";
...
function getOrderVolumeDisplayState(order) {
...
function getOrderActualStart(order, prevEndTime, isFirst) {
  const hasDate = !!(order.start_date || order._userSetStartDate);
  const hasTime = !!(order.start_datetime && order._userSetStartTime);
...
function calcOrderEnd(order, actualStart) {
  const volume = parseFloat(order.volume_override || order.total_volume_mt) || 0;
  const runRate = getLineRunRate(normalizeLine(order.feedmill_line));
  const calcProductionTime = runRate > 0 ? volume / runRate : 0;
  // Prefer the stored production_hours (same value shown on screen) over the
  // recalculated volume/runRate figure, which can differ if the order's hours
  // were set from batch counts or a different run-rate at save time.
  const productionTime = parseFloat(order.production_hours) > 0
...
function calculateEstimatedCompletionDates(orders) {
  ...
  for (let i = 0; i < enriched.length; i++) {
    ...
    const { end, productionTime } = calcOrderEnd(order, actualStart);
    enriched[i].production_hours = enriched[i].production_hours || productionTime;
...
function cascadeCompletionDates(updatedOrder, lineOrders) {
  ...
  for (let i = orderIndex; i < lineOrders.length; i++) {
    ...
    const { end, productionTime } = calcOrderEnd(order, actualStart);
    lineOrders[i].production_hours = lineOrders[i].production_hours || productionTime;
...
/* ─── applyPreviewChangeovers (mirrors AutoSequenceModal logic) ─── */
...
export function applyPreviewChangeovers(rows, changeoverRules) {
  ...
  rows.forEach((order, index) => {
    ...
    // Last order (no following order) gets zero changeover — nothing comes after it
...
function PlantBeforeRow({ order, allOrders, prio }) {
    ...
    <>
      <tr className={`auto-sequence-row-before${isLead ? ' as-row-combined-before' : ''}`}>
        ...
        <td className="as-col-material">
          <span title={order.material_code || ""} style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "#2e343a" }}>
            {order.material_code || "—"}
          </span>
        </td>
        ...
        <td className="as-col-prod">
          <div style={{ fontSize: 12, fontWeight: 600, color: "#1a1a1a" }}>{formatProductionHours(order.production_hours)} hours</div>
      ...
      {isExpanded && childOrders.map(child => (
        <tr key={`before-child-${child.id}`} className="as-row-combined-sub">
          ...
          <td className="as-col-material"><span className="as-sub-text">{child.material_code || "—"}</span></td>
          <td className="as-col-desc">
...
function PlantAfterRowContent({ order, provided, snapshot, prio, movement, movementDelta, inferredTargetMap, destinationLine, onUncombineSingle, onRemoveChildFromCombine, insight, isLoadingInsight, isProfitabilityApplied, showMarginCol, showProfitScoreCol, dateChange, onOrderUpdate, lineIndex, previousOrder }) {
  const [isExpanded, setIsExpanded] = useState(false);
  ...
  const userTime24 = (order._userSetStartTime && order.start_datetime)
      ...
      >
        ...
        <td className="as-col-prod">
          <div style={{ fontSize: 12, fontWeight: 600, color: "#1a1a1a" }}>
            {formatProductionHours(order.production_hours)} hours
          </div>
...
function buildChangeoverInsight(order, prevOrder, nextOrder, changeoverRules, line) {
  ...
  // Protected orders — CO optimisation doesn't reorder them
  if (mto || ['Critical', 'Urgent', 'Monitor'].includes(status)) {
    return {
  ...
  if (!nextOrder) {
    ...
    return {
      ...
      long:  `This order (${_materialLabel(order)}) closes the run, so its row-level changeover is 0.00 — nothing follows it.${prevLabel ? ` It is placed after the ${prevLabel} order.` : ''}${dateHint} Placing it last avoids adding any further downstream transition cost.${sDiff ? ` Difference from Standard: ${sDiff}` : ''}`,
    };
  }
...
export function PlantLineTab({ line, originalOrders, sequencedOrders, changeoverRules, inferredTargetMap, onOrdersChange,
  ...
  isRefreshingStrategies = false, onRefreshStrategies }) {
  ...
  return (
    <div className={`plant-line-tab${isRefreshingStrategies ? ' is-line-refreshing' : ''}`}>
...
export function PlantSummaryTab({ summaryStats, placementLog, aiExplanations, isLoadingAI, useFallback, onRefreshInsights, isProfitabilityApplied }) {
  return (
          ...
          <table className="plant-summary-table">
            <thead>
              <tr>
                ...
                <th className="text-right">Total HRS Before</th>
                <th className="text-right">Total HRS After</th>
                <th className="text-right">Net hours change</th>
...
function StrategyCard({ strategy, isSelected, isLoading, onSelect }) {
  const { metrics = {}, isAIRecommended, aiFailed, isLowDistinction, lowDistinctionReason } = strategy;
  const [showUtilDetails, setShowUtilDetails] = useState(false);
  const cls = [
    "ai-strategy-card",
    ...
    isLowDistinction && !aiFailed ? "ai-strategy-card-low-distinction" : "",
  ...
  return (
    ...
    >
      {(isAIRecommended || aiFailed || (isLowDistinction && !aiFailed)) && (
        <div className="ai-strategy-card-badges">
          {isLowDistinction && !aiFailed && (
            <span
              className="ai-strategy-badge ai-strategy-badge-low-distinction"
              title={lowDistinctionReason || 'This strategy is close to the Standard baseline.'}
              data-testid={`badge-low-distinction-${strategy.id}`}
            >
              ⓘ Low distinction
            </span>
          )}
          ...
          {isAIRecommended && !aiFailed && (
            <span
              className="ai-strategy-badge ai-strategy-badge-recommended"
              title="Recommended based on best overall balance of changeover improvement and reorder impact — among strategies that passed safety checks."
              data-testid={`badge-recommended-${strategy.id}`}
            >
              ⭐ Recommended
            </span>
          )}
      ...
      <div className="ai-strategy-header">
        ...
        <div className="ai-strategy-titles">
          <span className="ai-strategy-name">{strategy.name}</span>
          <span className="ai-strategy-theme">{strategy.theme}</span>
        </div>
      </div>

      <p className="ai-strategy-description">{strategy.description}</p>
      ...
      <div className="ai-strategy-metrics">
        ...
        */}
        <div className="ai-strategy-metric-row">
          <span className="ai-strategy-metric-label">MTS adjusted</span>
        ...
        <div className="ai-strategy-metric-row" data-testid={`row-changeover-${strategy.id}`}>
          <span className="ai-strategy-metric-label">Changeover time</span>
          <span className="ai-strategy-metric-value" data-testid={`text-changeover-${strategy.id}`}>
            {metrics.totalChangeoverHours != null
              ? `${metrics.totalChangeoverHours.toFixed(2)} hr`
              : '—'}
          </span>
        </div>
        <div className="ai-strategy-metric-row" data-testid={`row-timesaved-${strategy.id}`}>
          <span className="ai-strategy-metric-label">Time-saved</span>
          <span
            className={`ai-strategy-metric-value ${
              strategy.id === 'rule_based'
                ? 'ai-strategy-metric-neutral'
                : metrics.timeSavedDeltaHours < 0
                  ? 'ai-strategy-metric-good'
                  : metrics.timeSavedDeltaHours > 0
                    ? 'ai-strategy-metric-bad'
                    : 'ai-strategy-metric-neutral'
            }`}
            data-testid={`text-timesaved-${strategy.id}`}
          >
            {strategy.id === 'rule_based'
              ? '—'
              : metrics.timeSavedDeltaHours != null
                ? (() => {
                    const d = parseFloat(metrics.timeSavedDeltaHours) || 0;
                    return `${d > 0 ? '+' : ''}${d.toFixed(2)} hr`;
                  })()
                : '—'}
          </span>
        </div>
        <div className="ai-strategy-metric-row" data-testid={`row-orders-at-risk-${strategy.id}`}>
          <span className="ai-strategy-metric-label">Orders at risk</span>
          <span
            style={{ display: 'inline-flex', alignItems: 'center', gap: 0, fontSize: 12 }}
            data-testid={`text-orders-at-risk-${strategy.id}`}
          >
            {(() => {
              const count = metrics.ordersAtRisk ?? 0;
              const severity = metrics.riskSeverity ?? (count === 0 ? 'Low' : 'High');
        ...
        {(() => {
          const perDay = metrics.dailyUtilization?.perDay || [];
          const delta = parseFloat(metrics.utilizationDelta);
          const hasDelta = Number.isFinite(delta);
          const deltaClass = !hasDelta || delta === 0
            ? 'ai-strategy-metric-neutral'
            : delta > 0 ? 'ai-strategy-metric-good' : 'ai-strategy-metric-bad';
          ...
          const avgUtil = metrics.averageUtilization;
          const hasAvgUtil = avgUtil != null && Number.isFinite(avgUtil);
          return (
            <>
              <div
                className="ai-strategy-metric-row"
                data-testid={`row-daily-utilization-${strategy.id}`}
                onClick={(e) => { e.stopPropagation(); setShowUtilDetails(v => !v); }}
                style={{ cursor: perDay.length ? 'pointer' : 'default', userSelect: 'none' }}
              >
                <span className="ai-strategy-metric-label" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  Daily utilization
                  {perDay.length > 0 && (
                    <span style={{ fontSize: 10, lineHeight: 1, color: '#6b7280' }}>
                      {showUtilDetails ? '▾' : '▸'}
                    </span>
                  )}
                </span>
                <span
                  className={`ai-strategy-metric-value ${deltaClass}`}
                  data-testid={`text-daily-utilization-${strategy.id}`}
                >
                  {hasDelta
              ...
              {showUtilDetails && perDay.length > 0 && (
                <div
                ...
                >
                  ...
                  <div style={{
                    ...
                    <span>
                      ...
                      {isStandard ? (
                        <span style={{ color: '#6b7280', fontWeight: 500 }}>Baseline</span>
                      ) : improved ? (
                        <span style={{ color: '#15803d' }}>Improved</span>
                      ) : (
                    ...
                    {hasAvgUtil && (
                      <span
                        data-testid={`text-avg-utilization-${strategy.id}`}
                        style={{
                          fontSize: 11,
                          fontWeight: 700,
      ...
      {(strategy.aiReasoning || strategy.lineObservation || strategy.whyFits || strategy.differenceFromStandard || strategy.executionIntent || strategy.changeoverConsideration || strategy.mtsConsideration || strategy.profitabilityConsideration || strategy.tradeoffSummary) && (
        <details className="ai-strategy-reasoning" onClick={e => e.stopPropagation()}>
          <summary>View AI reasoning</summary>
          {metrics.ordersAtRisk != null && (
            <p data-testid={`text-risk-breakdown-${strategy.id}`}>
              <strong>Orders at risk:</strong>{' '}
              {(metrics.ordersAtRisk ?? 0) === 0
                ? 'None — no overdue or delayed orders in this sequence.'
                : `${metrics.ordersAtRisk} (${[
                    metrics.overdueCount ? `${metrics.overdueCount} overdue`  : null,
                    metrics.delayedCount ? `${metrics.delayedCount} delayed`  : null,
                  ].filter(Boolean).join(', ')})`
              }
            </p>
          )}
          {strategy.lineObservation && (
            <p data-testid={`text-line-observation-${strategy.id}`}>
              <strong>What this line shows:</strong> {strategy.lineObservation}
            </p>
          )}
          {strategy.whyFits && (
            <p data-testid={`text-why-fits-${strategy.id}`}>
              <strong>Why this strategy fits this line:</strong> {strategy.whyFits}
            </p>
          )}
          {(strategy.changeoverConsideration || strategy.mtsConsideration || strategy.profitabilityConsideration) && (
            <div className="ai-strategy-considerations" data-testid={`considerations-${strategy.id}`}>
              <div className="ai-strategy-considerations-title">Three-dimension analysis</div>
              {[
                { key: 'changeover',    label: 'Changeover reduction', data: strategy.changeoverConsideration },
                { key: 'mts',           label: 'MTS advancement',      data: strategy.mtsConsideration },
                { key: 'profitability', label: 'Profitability',        data: strategy.profitabilityConsideration },
              ].filter(d => d.data && (d.data.summary || d.data.relevance)).map(d => {
                const isPrimary = strategy.primaryEmphasis === d.key;
                const isSecondary = strategy.secondaryEmphasis === d.key;
                const isDeprioritized = Array.isArray(strategy.deprioritizedFactors) && strategy.deprioritizedFactors.includes(d.key);
                const rel = ['high', 'medium', 'low'].includes(d.data.relevance) ? d.data.relevance : 'medium';
                return (
                  <div
                    key={d.key}
                    className={`ai-strategy-consideration ai-strategy-consideration-${isPrimary ? 'primary' : isSecondary ? 'secondary' : isDeprioritized ? 'deprioritized' : 'neutral'}`}
                    data-testid={`consideration-${d.key}-${strategy.id}`}
                  >
                    <div className="ai-strategy-consideration-header">
                      <span className="ai-strategy-consideration-label">{d.label}</span>
                      {d.data.relevance ? (
                        <span className={`ai-strategy-relevance-pill ai-strategy-relevance-${rel}`}>
                          {rel}
                        </span>
                      ) : null}
                      {isPrimary    && <span className="ai-strategy-emphasis-tag ai-strategy-emphasis-primary">primary</span>}
                      {isSecondary  && <span className="ai-strategy-emphasis-tag ai-strategy-emphasis-secondary">secondary</span>}
                      {isDeprioritized && <span className="ai-strategy-emphasis-tag ai-strategy-emphasis-deprioritized">deprioritized</span>}
                    </div>
                    {d.data.summary && (
                      <div className="ai-strategy-consideration-summary">{d.data.summary}</div>
                    )}
                  </div>
                );
              })}
              {Array.isArray(strategy.deprioritizedFactors) && strategy.deprioritizedFactors.length > 0 && strategy.deprioritizationReason && (
                <p className="ai-strategy-deprioritization-reason" data-testid={`text-deprioritization-reason-${strategy.id}`}>
                  <strong>Why deprioritized:</strong> {strategy.deprioritizationReason}
                </p>
              )}
              {strategy.tradeoffSummary && (
                <p className="ai-strategy-tradeoff" data-testid={`text-tradeoff-${strategy.id}`}>
                  <strong>Tradeoff:</strong> {strategy.tradeoffSummary}
                </p>
              )}
            </div>
          )}
          {strategy.executionIntent && strategy.executionIntent.primary_goal && (
            <p data-testid={`text-execution-intent-${strategy.id}`}>
              <strong>Execution intent:</strong> {strategy.executionIntent.primary_goal.replace(/_/g, ' ')}
            </p>
          )}
          {SHOW_FAITHFULNESS && strategy.faithfulness && strategy.faithfulness.score != null && Number.isFinite(strategy.faithfulness.score) && (
            <p data-testid={`text-faithfulness-summary-${strategy.id}`}>
              <strong>Faithfulness:</strong>{' '}
              <span className={`ai-strategy-metric-faith-${
                strategy.faithfulness.score >= 90 ? 'high'
                : strategy.faithfulness.score >= 75 ? 'good'
                : strategy.faithfulness.score >= 60 ? 'medium'
                : 'low'
              }`}>
                {strategy.faithfulness.score.toFixed(1)}% — {strategy.faithfulness.rating}
              </span>
              {Array.isArray(strategy.faithfulness.notes) && strategy.faithfulness.notes.length > 0 && (
                <>{' '}— {strategy.faithfulness.notes[0]}</>
              )}
            </p>
          )}
          {strategy.differenceFromStandard && (
            <p data-testid={`text-difference-from-standard-${strategy.id}`}>
              <strong>Difference from Standard:</strong> {strategy.differenceFromStandard}
            </p>
          )}
          {strategy.aiReasoning && (
            <p data-testid={`text-reasoning-${strategy.id}`}>
              <strong>Reasoning:</strong> {strategy.aiReasoning}
            </p>
          )}
        </details>
      )}
...
function StrategySelector({ strategies, selectedId, isLoading, onSelect, line, isRefreshing = false, onRefresh }) {
...
export default function PlantAutoSequenceModal({
  ...
  const refreshLineStrategies = async (line) => {
    ...
    try {
      ...
      // ── Pre-build row insights for the refreshed line's strategies ────────
      // Mirror the pre-build done during the main auto-sequence run so that
      // after a per-line refresh the strategy-switch shortcut in PlantLineTab
      // still works (instant, no additional AI call on switch).
      await Promise.allSettled(
        ['rule_based', 'ai_option_1', 'ai_option_2'].map(async (stratId) => {
          const strat = refreshedWithStamp[stratId];
          if (!strat?.orders?.length) return;
          try {
            const insights = await generatePlantRowSequenceInsights({
              orders:                 strat.orders,
              line,
              strategyId:             stratId,
      ...
      // Guard: bail if token was replaced during insight generation.
      if (refreshTokensRef.current[line] !== token) return;
      ...
      <div className="plant-modal-container">
        ...
        <div className="plant-modal-header">
          <div>
            <div className="plant-modal-title">
              <Sparkles style={{ width: 18, height: 18, color: "#ea580c", marginRight: 8, flexShrink: 0 }} />
              {title}
            </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            {/* Single dynamic stats strip with profitability button */}
            <div className="plant-stats-strip" style={{ justifyContent: "space-between", flexWrap: "nowrap", gap: 12 }}>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
                {isLinetab && lineStats ? (
                  <>
                    {lineStats.fromOther > 0 && <StatBadge label="From Other Lines" value={lineStats.fromOther} color="green" />}
                    {lineStats.newlyCombined > 0 && <StatBadge label="Newly Combined" value={lineStats.newlyCombined} color="blue" />}
                    {lineStats.moved > 0 && <StatBadge label="Repositioned" value={lineStats.moved} />}
                    {lineStats.datesUpdated > 0 && <StatBadge label="Dates Updated" value={lineStats.datesUpdated} color="amber" />}
                    {!lineStats.hasChanges && lineStats.datesUpdated === 0 && <StatBadge label="Status" value="No Changes" />}
                  </>
                ) : summaryStats ? (
                  <>
                    ...
                    {summaryStats.ordersCombined > 0 && <StatBadge label="Combined Groups" value={summaryStats.ordersCombined} color="blue" />}
                    {summaryStats.ordersMovedBetweenLines > 0 && <StatBadge label="Cross-Line Moves" value={summaryStats.ordersMovedBetweenLines} color="green" />}
                    <StatBadge label="Lines Affected" value={summaryStats.linesAffected ?? 0} color="amber" />
                  </>
                ) : null}
              </div>
              {/* Profitability button — gated on the ACTIVE TAB's selection.
                  Disabled when the active line tab is showing an AI strategy
                  to avoid silently overwriting AI ordering. The summary tab
                  ungates the button (no specific line is "active"). */}
              {(() => {
                const activeLineSel = isLinetab ? selectedStrategyByLine[activeTab] : null;
                const aiActive = activeLineSel && activeLineSel !== 'rule_based';
                return (
                  <button
                    ...
                    title={aiActive
                      ? 'Switch to Standard Sequence to apply profitability sorting'
                      : isProfitabilityApplied
                        ? 'Click to revert to chronological sorting'
                        : 'Re-sort orders by product margin — higher margin = higher priority within same date'}
                    data-testid="button-profitability-toggle"
                  >
                    {isProfitabilityApplied ? '💰 Profitability Applied' : '💰 Apply Profitability (Margin)'}
                  </button>
                );
              })()}
            </div>
