...
export default function StatusDropdown({
  ...
  // Merge caller-supplied disabled values (e.g. Powermix source status gate)
  if (disabledValues.length > 0) {
    const disabledMap = Object.fromEntries(disabledValues.map(d => [d.value, d.hint || '']));
    dropdownOptions = dropdownOptions.map(s =>
      s.value in disabledMap
        ? { ...s, disabled: true, disabledHint: disabledMap[s.value] || s.disabledHint }
        : s
    );
  }
  ...
  return (
    <Popover open={open} onOpenChange={(o) => { if (!disabled) setOpen(o); }}>
      ...
      <PopoverContent
        className="status-dropdown-popover p-0 w-[210px] shadow-lg border border-gray-200 rounded-md overflow-hidden"
        align="start"
        sideOffset={4}
