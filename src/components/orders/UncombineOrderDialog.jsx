...
export default function UncombineOrderDialog({ open, onClose, onConfirm, leadOrder, childOrders = [] }) {
  ...
  const handleConfirm = async () => {
    ...
    console.debug('[Order Action Modal Confirm Click]', { modalType: 'uncombine', orderId, loadingStarted: true });
    setIsConfirming(true);
    console.debug('[Order Action Modal Loading State]', { modalType: 'uncombine', orderId, confirmButtonDisabled: true, loadingVisible: true });
    let success = false;
    try {
      await onConfirm();
      success = true;
    } catch (err) {
      console.error('[UncombineOrderDialog] confirm error', err);
      setIsConfirming(false);
    } finally {
      console.debug('[Order Action Modal Processing Complete]', { modalType: 'uncombine', orderId, success, loadingEnded: true });
    }
  };

  return (
    <AlertDialog open={open} onOpenChange={(o) => !o && !isConfirming && onClose()}>
