...
function getStatusCategory(status) {
  ...
  if (
    ...
    s === "ongoing_pelleting" ||
    s === "ongoing_bagging" ||
    s.startsWith("on_going")
  )
    return "in_progress";
  if (s === "hold") return "on_hold";
  if (s === "cancel_po") return "cancelled";
  return "scheduled";
}
...
const STATUS_SEGMENTS = [
  ...
  {
    key: "scheduled",
    color: "var(--scheduled-bar-color)",
    textColor: "var(--scheduled-bar-text)",
    label: "Scheduled",
    pdfLabel: "Scheduled",
  },
];
...
function calculateBreakdown(orders, month, year) {
  ...
  const buildBreakdown = (orderList) => {
    ...
    CATS.forEach((c) => {
      bd[`${c}_pct`] =
        bd.grandTotal > 0 ? Math.round((bd[c] / bd.grandTotal) * 100) : 0;
    });
    return bd;
  };
  ...
  Object.entries(FEEDMILL_LINES).forEach(([fm, lines]) => {
    ...
    const fmMonthOrders = monthOrders.filter((o) =>
      lines.includes(o.feedmill_line || o.line),
    );

    // ALL active orders across every month — used for totals and hasData
    const fmAllActive = orders.filter(
      (o) => lines.includes(o.feedmill_line || o.line) && !isInactiveOrder(o),
    );
    ...
    lines.forEach((line) => {
      const lineMonthOrders = fmMonthOrders.filter(
        (o) => (o.feedmill_line || o.line) === line,
      );
      const lineAllActive = fmAllActive.filter(
        (o) => (o.feedmill_line || o.line) === line,
      );
    ...
    >
      {STATUS_SEGMENTS.map((seg) => {
        ...
        return (
          <div
            key={seg.key}
            className={`stacked-bar-seg stacked-bar-${seg.key}`}
...
const SD_LINE_TO_FM = {};
Object.entries(FEEDMILL_LINES).forEach(([fm, ls]) =>
  ls.forEach((l) => {
    SD_LINE_TO_FM[l] = fm;
  }),
);
...
function sdFindKb(order, kbRecords) {
  ...
  const code = String(order.material_code_fg || order.material_code || "")
    .trim()
    .replace(/^0+/, "");
  return (
    kbRecords.find((p) => {
      const kc = String(p.fg_material_code || p.material_code_fg || "")
        .trim()
        .replace(/^0+/, "");
      ...
      {lines.map((line, index) => {
        ...
        if (trimmed.startsWith("•") || trimmed.startsWith("-")) {
          ...
          if (dashSplit.length >= 2) {
            ...
            let statusClass = "";
            // Risk keywords first (timeframe-aware), then fall back to legacy stock buckets
            if (
              dl.includes("high risk") ||
              dl.includes("miss avail") ||
              dl.includes("miss target") ||
              dl.includes("past avail") ||
              dl.includes("past target")
            )
              statusClass = "shutdown-order-critical";
            else if (dl.includes("medium risk") || dl.includes("delayed"))
              statusClass = "shutdown-order-urgent";
            else if (
              dl.includes("low risk") ||
              dl.includes("still safe") ||
              dl.includes("on time") ||
              dl.includes("still on time")
            )
              statusClass = "shutdown-order-sufficient";
            else if (dl.includes("critical"))
              statusClass = "shutdown-order-critical";
            else if (dl.includes("urgent"))
              statusClass = "shutdown-order-urgent";
            else if (dl.includes("monitor"))
              statusClass = "shutdown-order-monitor";
            else if (dl.includes("sufficient"))
              statusClass = "shutdown-order-sufficient";
            return (
        ...
        if (
          lc.startsWith("capacity note") ||
          lc.startsWith("partner line capacity") ||
          lc.startsWith("total impact")
        ) {
          return (
            <div key={index} className="shutdown-analysis-note">
              {trimmed}
            </div>
          );
        }
        ...
        if (trimmed.endsWith(":") && trimmed.length < 60) {
          return (
            <div key={index} className="shutdown-analysis-subheader">
              {trimmed}
            </div>
          );
        }
        ...
        return (
          <p key={index} className="shutdown-analysis-paragraph">
            {trimmed}
          </p>
        );
      })}
...
function ShutdownDialogAI({
  ...
  // Default downtime window: now → now + 2h (matches client example "6am to 8am")
  const _pad = (n) => String(n).padStart(2, "0");
  const _fmtLocal = (d) =>
    `${d.getFullYear()}-${_pad(d.getMonth() + 1)}-${_pad(d.getDate())}T${_pad(d.getHours())}:${_pad(d.getMinutes())}`;
  const _now = new Date();

  const [shutdownType, setShutdownType] = useState("unplanned");
  const [reason, setReason] = useState("");
  const [notes, setNotes] = useState("");
  const [startDateTime, setStartDateTime] = useState(_fmtLocal(_now));
  const [endDateTime, setEndDateTime] = useState(
    _fmtLocal(new Date(_now.getTime() + 2 * 3600000)),
  );
  ...
  const validTimeframe = !isNaN(startMs) && !isNaN(endMs) && endMs > startMs;
  const durationHours = validTimeframe
  ...
  const allShutdownLines = [
    ...affectedLines,
    ...Object.entries(lineShutdowns)
      .filter(([, info]) => info.isShutdown)
      .map(([l]) => l),
  ];
  const uniqueShutdownLines = [...new Set(allShutdownLines)];
  const partnerLines = fmLines.filter(
    (l) => !affectedLines.includes(l) && !uniqueShutdownLines.includes(l),
  );
  const allLineKeys = Object.keys(SD_LINE_TO_FM);
  const outsideAvailableLines = allLineKeys.filter(
    (l) => !fmLines.includes(l) && !uniqueShutdownLines.includes(l),
  );

  const affectedOrders = orders.filter((o) => {
    const ol = o.feedmill_line || o.line || "";
    return (
      affectedLines.includes(ol) &&
      !["done", "completed", "cancel_po", "cancelled"].includes(
    ...
    async function runAnalysis() {
      ...
      try {
        ...
        partnerLines.forEach((pl) => {
          const plOrders = orders.filter(
            (o) =>
              (o.feedmill_line || o.line) === pl &&
              !["done", "completed", "cancel_po", "cancelled"].includes(
                (o.status || "").toLowerCase(),
              ),
          );
          partnerLineLoad[pl] = plOrders.length;
          partnerLineVolume[pl] = plOrders
            .reduce((s, o) => s + getVolume(o), 0)
            .toFixed(0);
        });
        ...
        outsideAvailableLines.forEach((ol) => {
          const olOrders = orders.filter(
            (o) =>
              (o.feedmill_line || o.line) === ol &&
              !["done", "completed", "cancel_po", "cancelled"].includes(
                (o.status || "").toLowerCase(),
              ),
          );
          outsideLineLoad[ol] = olOrders.length;
          outsideLineVolume[ol] = olOrders
            .reduce((s, o) => s + getVolume(o), 0)
            .toFixed(0);
        });

        // Per-line cumulative completion estimates (used as fallback when an
        // order has no explicit target_completion_date).  Active orders on each
        // affected line are sorted by priority_seq and projected forward from
        // "now" using each order's run rate.
        const completionEstimateById = new Map();
        affectedLines.forEach((line) => {
          const queue = orders
            .filter(
              (o) =>
                (o.feedmill_line || o.line) === line &&
                !["done", "completed", "cancel_po", "cancelled"].includes(
                  (o.status || "").toLowerCase(),
                ),
            )
            .sort(
              (a, b) => (a.priority_seq ?? 999999) - (b.priority_seq ?? 999999),
            );
          let cumHours = 0;
          for (const o of queue) {
            ...
            const v = getVolume(o);
            const rr =
              parseFloat(o.run_rate) ||
              (kb
                ? parseFloat(
                    kb[SD_LINE_RATE_KEYS[o.feedmill_line || o.line]] || 0,
                  )
                : 0);
            const ph = rr > 0 ? v / rr : 0;
            cumHours += ph;
            completionEstimateById.set(o.id, Date.now() + cumHours * 3600000);
          }
        });

        // Local-date formatter (avoids UTC ±1 day drift from toISOString)
        const fmtDate = (ms) => {
        ...
        // Treat an avail date as the END of that local day, so an order
        // completing at 5pm on its avail date is still considered on time.
        const parseAvailEOD = (s) => {
          if (!s) return null;
        ...
        // Enrich affected orders with timeframe-aware risk
        const enrichedOrders = affectedOrders.map((o) => {
          ...
          const outsideLines = outsideAvailableLines
            .filter((l) => lineRateMap[l] > 0)
            .map((l) => `${l} (rate: ${lineRateMap[l]} MT/hr)`);
          const canDivertOutside = outsideLines.length > 0;
          const canDivertWithin = partnerLines.length > 0;
          const canBeDiverted = canDivertWithin || canDivertOutside;

          const inf =
            inferredTargetMap[o.material_code_fg || o.material_code] || null;
          ...
          const productionHours =
            runRate > 0 ? Number((volume / runRate).toFixed(2)) : 0;

          // Use target_completion_date if present, else derived cumulative estimate
          const tcdMs = o.target_completion_date
            ? new Date(o.target_completion_date).getTime()
            : null;
          const currentEstCompletionMs =
            tcdMs && !isNaN(tcdMs)
              ? tcdMs
              : (completionEstimateById.get(o.id) ?? null);
          // Shutdown only delays orders whose original completion is during/after the shutdown start
          const adjustedEstCompletionMs =
            currentEstCompletionMs != null
              ? currentEstCompletionMs > startMs
                ? currentEstCompletionMs + durationHours * 3600000
                : currentEstCompletionMs
              : null;

          const availDateStr = o.target_avail_date || o.avail_date || null;
          const availMs = parseAvailEOD(availDateStr);
          const missesAvailDateAfterShutdown =
            availMs && adjustedEstCompletionMs
              ? adjustedEstCompletionMs > availMs
              : false;

          let riskLevel; // 'high' | 'medium' | 'low'
          if (missesAvailDateAfterShutdown && !canBeDiverted)
            riskLevel = "high";
          else if (missesAvailDateAfterShutdown && canBeDiverted)
            riskLevel = "medium";
          else if (
            availMs &&
            adjustedEstCompletionMs &&
            availMs - adjustedEstCompletionMs < 24 * 3600000
          )
            riskLevel = "medium";
          else riskLevel = "low";

          console.debug("[Shutdown Order Impact Analysis]", {
            orderId: o.id,
            orderName: o.item_description || o.item || "—",
            productionHours,
            currentEstimatedCompletion: fmtDate(currentEstCompletionMs),
            adjustedEstimatedCompletion: fmtDate(adjustedEstCompletionMs),
            availDate: availDateStr,
            missesAvailDateAfterShutdown,
            canBeDiverted,
          });
        ...
        // Sort: high risk → medium → low, then n10d bucket as tiebreak
        ...
        const ordersAtRisk = enrichedOrders.filter(
          (o) => o.riskLevel === "high" || o.riskLevel === "medium",
        ).length;
        const ordersStillSafe = enrichedOrders.filter(
          (o) => o.riskLevel === "low",
        ).length;
        const diversionCandidates = enrichedOrders.filter(
          (o) =>
            o.canBeDiverted &&
        ...
        console.debug("[Shutdown Impact Summary]", {
          lineId: target,
          activeOrdersAffected: enrichedOrders.length,
          ordersAtRisk,
          ordersStillSafe,
          diversionCandidates,
        });

        const totalProductionHours = enrichedOrders
          .reduce((s, o) => s + (parseFloat(o.productionHours) || 0), 0)
          .toFixed(1);
        ...
        setAnalysis(
          "Unable to generate analysis. You can still proceed with the shutdown.",
        );
      } finally {
        setIsAnalyzing(false);
      }
    }
    // Debounce so each datetime-picker keystroke doesn't fire an AI call
    let cancelled = false;
  ...
  }, [refreshCount, validTimeframe, startMs, endMs, shutdownType]);

  // Clear stale analysis immediately when inputs change so the user never
  // confirms a shutdown against analysis text generated for an older timeframe.
  useEffect(() => {
    setAnalysis(null);
  }, [startMs, endMs, shutdownType]);

  const canSubmit =
    reason !== "" && shutdownType !== "" && validTimeframe && !isAnalyzing;

  // Track modal width for responsive two-column / stacked fallback
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      for (const e of entries) setModalWidth(e.contentRect.width);
    });
    ro.observe(el);
    return () => ro.disconnect();
  }, []);
  const isSideBySide = modalWidth >= 640;
  const layoutMode = isSideBySide ? "side_by_side" : "stacked_fallback";

  console.debug("[Shutdown Modal Layout]", {
    layoutMode,
    modalWidth,
    leftPanelRendered: true,
    rightPanelRendered: true,
  });

  console.debug("[Shutdown Impact Live Refresh]", {
    shutdownType,
    reason,
    startDateTime,
    endDateTime,
    impactAnalysisRecomputed: !isAnalyzing && analysis !== null,
  });
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            {feedmill && (
              ...
              >
                {FM_DISPLAY_MAP[feedmill] || feedmill}
              </span>
            )}
          </div>
          <button
        ...
        >
          ...
          <div
            style={{
              flexShrink: 0,
              width: isSideBySide ? "340px" : "100%",
              maxHeight: isSideBySide ? "100%" : "none",
              overflowY: "auto",
              borderRight: isSideBySide
                ? "1px solid var(--color-border)"
                : "none",
              borderBottom: !isSideBySide
                ? "1px solid var(--color-border)"
                : "none",
              display: "flex",
              flexDirection: "column",
            ...
            >
              ...
              <div>
                <label style={labelStyle}>
                  Shutdown Type <span style={{ color: "#dc2626" }}>*</span>
                </label>
                <select
                  value={shutdownType}
                  onChange={(e) => setShutdownType(e.target.value)}
                  data-testid="select-shutdown-type"
                  style={inputStyle}
                >
              ...
              <div>
                ...
                <select
                  ...
                  data-testid="select-shutdown-reason"
                  style={inputStyle}
                >
              ...
              <div>
                <label style={labelStyle}>
                  Downtime Window <span style={{ color: "#dc2626" }}>*</span>
                </label>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      data-testid="input-shutdown-start"
                      style={inputStyle}
                    />
                  </div>
                  <div>
                    <span style={subLabelStyle}>End</span>
                    <input
                      type="datetime-local"
                      value={endDateTime}
                      onChange={(e) => setEndDateTime(e.target.value)}
                      data-testid="input-shutdown-end"
                      style={{
                        ...inputStyle,
                        border: `1px solid ${validTimeframe ? "#d1d5db" : "#dc2626"}`,
                      }}
                    />
                  </div>
                </div>
                <div
                  style={{
                    ...
                    fontSize: "11px",
                    fontWeight: 500,
                    color: validTimeframe ? "#16a34a" : "#dc2626",
                    display: "flex",
                    alignItems: "center",
                    gap: "4px",
                  }}
                  data-testid="text-shutdown-duration"
                >
                  {validTimeframe ? (
                    <>
                      <span style={{ fontSize: "13px" }}>⏱</span> Duration:{" "}
                      {durationHours} hour{durationHours === 1 ? "" : "s"}
                    </>
                  ) : (
                    <>
                      <span>⚠</span> End must be after Start
                    </>
                  )}
                </div>
              </div>
          ...
          >
            ...
            {/* AI analysis sub-header */}
            <div
              className="shutdown-impact-header"
              style={{
                flexShrink: 0,
                display: "flex",
                justifyContent: "space-between",
            ...
            <div
              className="shutdown-analysis-scroll"
            ...
            >
              {isAnalyzing && !analysis ? (
                ...
                >
                  Analyzing impact on orders and identifying diversion
                  opportunities...
                </span>
              ) : analysis ? (
            ...
            {!isAnalyzing && partnerLines.length === 0 && (
              ...
              >
                <span>ℹ</span>
                No other lines in {FM_DISPLAY_MAP[feedmill] || feedmill}{" "}
                available for diversion. Orders remain on this line until
                resumed.
              </div>
            )}
          </div>
...
function FeedmillCard({
  ...
  // Use activeBreakdown as fallback when no month-scoped data exists — keeps
  // the Category Breakdown rows consistent with the top stacked bar and Total row.
  const displayBreakdown =
    fmData && fmData.grandTotal > 0
      ? fmData
      : fmData?.activeBreakdown || fmData;
  const maxCat = hasData
    ? Math.max(...STATUS_SEGMENTS.map((s) => displayBreakdown?.[s.key] || 0))
    : 0;
  ...
  return (
    <>
      <div
        style={{
          background: "var(--color-bg-secondary)",
          border: `1px solid ${isFeedmillShutdown ? "#fca5a5" : "var(--color-border)"}`,
          borderRadius: "8px",
          overflow: "hidden",
          boxShadow: isFeedmillShutdown
      ...
      >
        ...
        {isFeedmillShutdown && (
          <div
            className="feedmill-shutdown-banner"
            style={{
              background: "#fef2f2",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              <span
                style={{ fontSize: "11px", fontWeight: 600, color: "#b91c1c" }}
              >
                SHUTDOWN
              </span>
              {lines[0] && lineShutdowns[lines[0]]?.since && (
                <span style={{ fontSize: "10px", color: "#ef4444" }}>
                  since{" "}
                  {new Date(lineShutdowns[lines[0]].since).toLocaleDateString()}
                </span>
              )}
            </div>
            <button
              onClick={() =>
                setResumeConfirm({
                  type: "feedmill",
                  displayName: FM_DISPLAY[fm] || fm,
                })
              }
              className="feedmill-resume-all-btn"
              style={{
                fontSize: "10px",
                color: "#16a34a",
                fontWeight: 600,
        ...
        >
          <span
            style={{
              fontSize: "13px",
              fontWeight: 700,
              color: isFeedmillShutdown ? "#b91c1c" : "var(--color-text)",
            }}
          >
            {FM_DISPLAY[fm] || fm}
          </span>
        ...
        <div style={{ padding: "12px 16px" }}>
          {!hasData ? (
            ...
            <>
              ...
              <div
                style={{ fontSize: "11px", fontWeight: 500, marginTop: "6px" }}
              >
                <span style={{ color: "var(--color-text-muted)" }}>
                  {fmData.total.toLocaleString(undefined, {
              ...
              {expanded && (
                <>
                        ...
                        >
                          ...
                          {isLineDown ? (
                            <button
                              ...
                              className="shutdown-line-resume-button"
                              style={{
                                display: "flex",
                                alignItems: "center",
                                gap: "3px",
                            ...
                            <button
                              ...
                              className="shutdown-line-button"
                              style={{
                                display: "flex",
                                alignItems: "center",
                                gap: "3px",
                        ...
                        {ld && ld.total > 0 ? (
                          <>
                            ...
                            >
                              ...
                              >
                                {ld.total.toLocaleString(undefined, {
                                  maximumFractionDigits: 1,
                                })}{" "}
                                MT
                              </span>
                              ...
                              >
                                {" "}
                                · {CAPACITY_MAP[fm]?.lines[line] ?? 0} MT/hr
                              </span>
                            </div>
                          </>
                        ) : (
                  ...
                  {/* Spacer for feedmills with only 1 line (e.g. Powermix/Line 5) so the
                      Category Breakdown heading visually aligns with the other feedmill cards
                      which each have 2 lines. Each line section is ~80px tall. */}
                  {lines.length < 2 && <div style={{ height: "65px" }} />}
                  ...
                  {(() => {
                    console.debug("[Category Breakdown Layout]", {
                      container: FM_DISPLAY[fm] || fm,
                      section: "Category Breakdown",
                      alignedWithStandardFeedmillLayout: true,
                    });
                    console.debug("[Category Breakdown Row Alignment]", {
                      container: FM_DISPLAY[fm] || fm,
                      categories: STATUS_SEGMENTS.map((s) => s.label),
                    });
                    return null;
                  })()}
                  {STATUS_SEGMENTS.map((seg) => {
                    const mt = displayBreakdown?.[seg.key] || 0;
        ...
        <ShutdownDialogAI
          ...
          onConfirm={(shutdownData) => {
            if (activeShutdown.targetType === "line") {
              onShutdownLine &&
                onShutdownLine(activeShutdown.target, fm, shutdownData);
            } else {
            ...
            >
              ...
              >
                ...
                {resumeConfirm.type === "feedmill" && (
                  ...
                  >
                    This will mark all lines in {resumeConfirm.displayName} as
                    active and clear their shutdown status.
                  </p>
                )}
              </div>
              {/* Footer */}
                  ...
                  onClick={() => {
                    if (resumeConfirm.type === "line") {
                      ...
                      onResumeFeedmill
                        ? onResumeFeedmill(fm)
                        : lines.forEach(
                            (l) => onResumeLine && onResumeLine(l, fm),
                          );
                    }
  ...
  lines.forEach((line) => {
    ...
    if (isHeading) {
      ...
      wrapped.forEach((wl, i) =>
        processed.push({ type: "bullet", text: wl, indent: i === 0 }),
      );
      processed.push({ type: "spacer", h: 2 });
    } else {
...
async function generatePDF(breakdown, monthOrders, monthLabel, aiInsight) {
  ...
  const addFooters = () => {
    ...
    for (let i = 1; i <= total; i++) {
      ...
      pdf.setTextColor(...GREY);
      pdf.text(
        "Generated by NexFeed Production Scheduling System",
        PAGE_W / 2,
        290,
        { align: "center" },
      );
      pdf.text(`Page ${i} of ${total}`, PAGE_W - MARGIN, 290, {
        align: "right",
      });
    }
  };
  ...
  pdf.setFont("helvetica", "normal");
  pdf.setTextColor(...DARK);
  pdf.text(`Total Orders: ${monthOrders.length}`, MARGIN, y);
  y += 5;
  pdf.text(`Total Volume: ${fmt(overall.total)} MT`, MARGIN, y);
  y += 8;

  autoTable(pdf, {
    ...buildAutoTableOptions(pdf, buildSummaryRows(overall), MARGIN),
    startY: y,
  });
  ...
  for (const [fm, lines] of Object.entries(FEEDMILL_LINES)) {
    ...
    const fmData =
      breakdown[fm] ||
      Object.fromEntries([...CATS.map((c) => [c, 0]), ["total", 0]]);
    const fmLabel =
      fm === "PMX"
        ? "POWERMIX (PMX)"
        : `FEEDMILL ${fm.replace("FM", "")} (${fm})`;
    ...
    autoTable(pdf, {
      ...buildAutoTableOptions(pdf, buildSummaryRows(fmData), MARGIN),
      startY: y,
    });
  ...
  const sorted = [...monthOrders].sort((a, b) => {
    ...
    return (
      la.localeCompare(lb) || (a.priority_seq || 0) - (b.priority_seq || 0)
    );
  });
...
// ─── RecentShutdowns ──────────────────────────────────────────────────────────
const FM_LABEL = { FM1: "FM1", FM2: "FM2", FM3: "FM3", PMX: "PMX" };

function RecentShutdowns({ shutdownHistory = [] }) {
  const latest = shutdownHistory.slice(0, 10);
  const recordCount = latest.length;

  console.debug("[Overview Shutdown History Load]", {
    recordCount,
    latestShutdowns: latest,
  });
  console.debug("[Overview Shutdown History Render]", {
    sectionVisible: true,
    renderedRows: recordCount,
  });
  ...
  return (
    <div>
      ...
      >
        <span>⏻</span>
        <span>Recent Shutdowns</span>
        {recordCount > 0 && (
          <span className="recent-shutdowns-badge">{recordCount}</span>
        )}
      </div>
      ...
      {recordCount === 0 ? (
        <div
          ...
          data-testid="text-no-shutdown-history"
        >
          <span style={{ fontSize: "28px" }}>⏻</span>
          <span style={{ fontSize: "13px", fontWeight: 500 }}>
            No shutdown history yet
          </span>
          <span style={{ fontSize: "12px" }}>
            Shutdown records will appear here after a line is shut down.
          </span>
        </div>
      ) : (
        <div style={{ overflowX: "auto" }}>
          ...
          >
            ...
            <tbody>
              {latest.map((rec, i) => {
                const isPlanned = rec.shutdownType === "planned";
                return (
                  <tr
                    key={rec.id || i}
                    data-testid={`row-shutdown-history-${i}`}
                    style={{
                      borderBottom: "1px solid var(--color-border)",
                      background:
                        i % 2 === 0
                  ...
                  >
                    ...
                    >
                      {FM_LABEL[rec.feedmill] || rec.feedmill || "—"}
                    </td>
                    {/* Type badge */}
                    <td style={{ padding: "8px 10px", whiteSpace: "nowrap" }}>
                      <span
                        style={{
                          ...
                          padding: "2px 7px",
                          borderRadius: "20px",
                          background: isPlanned ? "#dbeafe" : "#fee2e2",
                          color: isPlanned ? "#1e40af" : "#b91c1c",
                        }}
                      >
                    ...
                    >
                      {rec.startDateTime && rec.endDateTime
                        ? `${fmtDate(rec.startDateTime)} → ${fmtDate(rec.endDateTime)}`
                        : rec.startDateTime
                          ? `From ${fmtDate(rec.startDateTime)}`
                          : "—"}
                    </td>
                    {/* Duration */}
                    <td
                    ...
                    >
                      {rec.durationHours > 0
                        ? `${rec.durationHours} hr${rec.durationHours === 1 ? "" : "s"}`
                        : "—"}
                    </td>
                  </tr>
                );
...
function ProductionStatusMonitoring({
  breakdown,
  monthOrders,
  monthLabel,
  rollingMonths,
  ...
  return (
    <div>
      ...
      {/* Feedmill cards */}
      <div
        className="production-status-grid"
        data-tour="overview-feedmill-cards"
        style={{
          display: "grid",
      ...
      >
        {STATUS_SEGMENTS.map((seg) => (
          ...
          >
            <span
              className={`stacked-bar-legend-swatch stacked-bar-legend-${seg.key}`}
              style={{
                display: "inline-block",
                width: "10px",
                height: "10px",
...
export default function OverviewDashboard({
  ...
  const runGenerateInsight = useCallback(
    async (bd, mo, mon, yr) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!insight) {
        const mon = rollingMonths[selectedIdx].label.split(" ")[0];
        insight = await generateReportInsight(
          breakdown,
          mon,
          year,
          monthOrders,
  ...
  const urgentOrders = orders.filter((order) => {
    ...
    if (INACTIVE_STATUSES.includes((order.status || "").toLowerCase()))
      return false;
    // 1. Safety stock Critical or Urgent status always counts
    const inf =
      inferredTargetMap[order.material_code] ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div style={{ marginBottom: "0" }} data-tour="overview-insights">
        ...
        >
          ...
          <button
            ...
            onMouseEnter={(e) => {
              if (!isLoadingInsight) e.currentTarget.style.color = "#c2410c";
            }}
            onMouseLeave={(e) => {
      ...
      {/* ── 4. Recent Shutdowns ── */}
      <Card className="border-0 shadow-sm" data-testid="card-recent-shutdowns">
        <CardContent className="p-6">
          <RecentShutdowns shutdownHistory={shutdownHistory} />
        </CardContent>
      </Card>
